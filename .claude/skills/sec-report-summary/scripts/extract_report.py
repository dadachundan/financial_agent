...
Usage:
    ...
    python3 extract_report.py --id 682 --deep        # uncapped sections

For 10-K it returns Item 1 (Business) + Item 1A (Risk Factors) + Item 7
(MD&A). For 10-Q it returns Item 2 (MD&A) + Item 1A Part II. For 8-K it
returns substantive items (1.01, 2.01, 2.02, 8.01).
...
def main() -> None:
    ...
    ap.add_argument("--max-section", type=int, default=DEFAULT_MAX_SECTION,
                    help=f"per-section cap (default {DEFAULT_MAX_SECTION:,}); "
                         f"each Item 1 / 1A / 7 / MD&A is truncated to this. "
                         f"Ignored when --deep is set.")
    ap.add_argument("--deep", action="store_true",
                    help="disable per-section truncation — return the full "
                         "Item 1 / 1A / 7 / MD&A text. Use for cross-year "
