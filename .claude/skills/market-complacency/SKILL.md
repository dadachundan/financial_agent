...
- **Multpl S&P 500 earnings & dividend lag (known limitation).** [multpl.com/s-p-500-earnings](https://www.multpl.com/s-p-500-earnings) and the dividend-yield variant update **only when full GAAP earnings are finalized**, so their "latest" data point can lag by 6-12 months. As of June 2026, multpl's latest TTM EPS is **Sep 30, 2025 = $239.98** — 8 months stale. The true current TTM EPS is ~$245-250 based on (a) SPY trailing PE 26.66 × SPY price $737.55 → implied SPY EPS $27.67 → S&P 500 EPS ~$245, and (b) FactSet's reported +27.7% YoY Q1 2026 earnings growth. **Every report run must** (i) check multpl's latest date and note it in italics next to the EPS row in Figure 2; (ii) compute an "implied current TTM EPS" using `SPY price ÷ SPY trailingPE × ~9` to disclose the freshness gap. The **rolling-10y-peak distance percentage doesn't materially change** because both numerator and denominator are stale by the same amount — the indicator's complacency flag holds. CAPE / ERP / DY / forward-PE indicators that use ratios are similarly less affected. See the EPS row caveat in `reports/market-complacency/market_complacency_2026-06-07.md` for the canonical italic-caveat pattern.
