...
**Lead with a Header Data Block, not prose (Step 5 / new lead element).** Mirror Jefferies/JPM/MS/UBS/Bernstein: open with a compact key-value block — **Rating | 12m Price Target (current vs. PRIOR) with % to PT | last price (as-of date) | 52-week high–low | market cap | ADV** — then a multi-year FY estimate strip (**Revenue / EBIT or EBITDA / Adj Net Profit / EPS for 2 actual + 2 estimate years**). State the valuation method on the PT line (e.g. "34x next-FY EPS", "SOTP", "DCF WACC 12%"). Keep the standing Rating/PT/upside line near the top so the reader sees the call without scrolling.
