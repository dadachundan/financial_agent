...
2. **5–9 visuals embedded — never matplotlib** (disabled 2026-06-03 for memory; see SKILL.md § Step 8). Two memory-safe systems: **(A) `scripts/financial_charts.py`** inline SVG for the financials — at minimum the **income-statement Sankey** + one revenue **donut**; add balance / cashflow Sankeys, `revbars`, and the `dupont` ROE tree by fit; **plus the REQUIRED `moneyflow` supply-chain map (one per report)** (paste the `<svg>` **un-fenced**; the `--source` is baked in; every number traces to the company's own 10-K/10-Q/IR statement you cite — see [`financial_charts.md`](financial_charts.md) and [`money_flow.md`](money_flow.md)). **(B) Mermaid** ` ```mermaid ` fences for `timeline`, `graph TD`, `quadrantChart`, and simple `xychart-beta` trends. Each chart has a markdown-link citation directly beneath it. **No Mermaid `xychart-beta` mixes units (%, currency, count) on one y-axis** — split revenue and margin into two stacked charts (see SKILL.md § Step 8). (Legacy PNGs from before 2026-06-03 may be reused in their original reports.)
   - [ ] **Financial-statement SVGs render** — each `financial_charts.py` `<svg>` is pasted **un-fenced** (not inside a ``` block) so it displays as a chart; the `--source` footer cites the exact statement; the surrounding paragraph carries the page-level citation; no chart contains a figure not traceable to a cited filing.
   - [ ] **Money-flow diagram present & sourced** — the `moneyflow` `<svg>` is pasted **un-fenced**; every node is a REAL counterpart sourced from a filing / IR / teardown-channel report / the zsxq library (no invented suppliers); any number in a ribbon label OR a "Follow the money" card body string-matches a source cited in the surrounding paragraph; the `--source` footer is present; a short "follow the money" caption names the chokepoint(s) and cites each link. (Drop only when the chain genuinely can't be sourced — noted in the Step-10 log.)
