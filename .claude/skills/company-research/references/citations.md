...
- **Forward-looking analyst inference must still carry an external hyperlink.** When you write "we estimate the SAM at $4bn" or "we project FY26E revenue of X", the estimate is built on something — the company's latest 10-K, recent earnings guidance, an industry forecast. Cite that external basis in the same parenthetical: `(estimate based on [10-K FY2024](https://...) + [Gartner 2025 forecast](https://...))`. Avoid a standalone `(model estimate)` label with no clickable URL — readers should always have at least one external source to verify the basis the estimate stands on.
