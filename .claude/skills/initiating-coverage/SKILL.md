...
**No solo model labels — hybrid factual + model citation rule.** A paragraph that mentions a forward-looking projection (FY26E, FY27E, our model, base case, scenario, sensitivity) AND also references a historical/external fact (FY2025 results, 20-F section, Yole, peer comparison) must get a HYBRID citation: `(来源: [external source 1](rId) | [external source 2](rId) | 本报告模型估算)`. The external sources lead, the model label trails. Pure forward-looking paragraphs with no external reference default to `(来源: [FY2024 20-F 为基线](rId44) | 本报告模型估算)` — the 20-F is the baseline the model is built on, always linkable. Standalone `(来源: 本报告模型估算)` with no hyperlink is reserved for financial-model-tab references (DCF, scenarios, sensitivity tables) where the source genuinely IS the analyst's internal workbook with no external anchor.
