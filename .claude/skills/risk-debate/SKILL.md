...
When this debate hinges on something a reader would struggle to picture from prose alone — the hard-to-visualize mechanism behind a risk being argued (e.g. the physical failure mode of a manufacturing process the Bear cites, the architecture of the product whose obsolescence the Conservative analyst fears, a market-structure or derivatives mechanic underlying the trigger ladder, or an unfamiliar business model whose unit economics the voices disagree on) — attach **1–3 short explainer videos** (YouTube and/or Bilibili) so the reader can *see* the risk mechanism, not just read about it. Default to including them whenever a debated risk has a tangible mechanism worth visualizing; omit only when the debate is purely numeric with nothing worth visualizing.
