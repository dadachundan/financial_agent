...
**Hardware: 16 GB Apple M4 MacBook Air (`xs-macbook-air.local`).** Only ~8 GB free at idle (Claude app + the user's Flask `:5001` + OS). Fan-out is what OOMs this box, not the skills themselves. Hard rules — do not violate:

1. **Run `/company-research` and every 6–10k-word report skill FLEET at concurrency 1 — strictly sequential.** One report agent at a time; launch the next only after the prior returns. Concurrency 2 is marginal (only if free RAM >60% and Flask not running); 3+ OOMs.
2. **Prefer `Agent`-tool subagents (separate processes) over in-process Workflow `agent()`** for report fleets — isolated heap, full reclaim on exit, peak bounded to `main + 1 subagent`.
3. **Never launch a ≥2-wide fan-out (parallel `Agent` calls or Workflow `parallel()`, or chaining >3 report agents) without the `/tmp/mem-watch-16gb.sh` watcher running.** Check `pgrep -lf 'mem-watch-16gb.sh'` first.
4. **At DANGER (<12% free RAM) stop launching immediately** (and `TaskStop` any in-flight Workflow) — swap does not free up until agents finish. **Stop the watcher when the chain finishes** (`pkill -f mem-watch-16gb.sh`).
5. **Never report a chain as "running fine" without checking `/tmp/mem-watch.log` AND `pgrep -lf 'mem-watch-16gb.sh'`** — the watcher's status column is the source of truth.

Full procedure — the watcher script, free-%-based thresholds, concurrency-sizing tables, the launch-message template, and OOM/empty-folder-debris recovery — is in the **`heavy-workflow-memory` skill**. Load it before launching any heavy fan-out.
...
After adding or modifying any UI feature (buttons, modals, navigation), verify end-to-end before considering it done: **start the real server on port 5002 (never 5001 — that port belongs to the user's running instance), click every new button, trace JS errors, verify navigation flows, prove each interaction with a screenshot/eval, then stop the server (`preview_stop` + `lsof -ti :5002 | xargs kill -9`).** Full procedure in the **`ui-verification` skill**.
...
When the user asks to make a table field editable, always use the `md_comment_widget.py` pattern (preview modal → EasyMDE editor with image upload → POST JSON `{"description": "..."}` → re-render the cell in place, no reload). Full steps, the shared blueprint (`/upload-image`, `/uploads/<path>`), and the reference implementation (`viewEntityDesc` in `templates/index.html`) are in the **`editable-table-columns` skill**.
...
When a PDF page returns empty text from `fitz.get_text()`, use the three-tier OCR flow — **never Tesseract**: (1) `ocrmac` (Apple Vision) by default, (2) Marker for layout/tables, (3) Claude vision (`render_pdf_pages.py` → Read the PNG) only for charts. The wrapper `.claude/skills/zsxq-analyze/scripts/ocr_pdf.py` caches results to `pdf_files.ocr_text`/`ocr_at`. Full details in the **`pdf-text-extraction` skill**.
...
All markdown reports under `reports/` (company / sector / compare / earnings / unlisted / etc.) must meet the project's paragraph-level citation standard. The authoritative spec is `reference/citations.md`. Summary:
...
- **Agent prompt template** — `reports/_backfill_agent_template.md`. Each backfill agent reads it + `reference/citations.md`, audits paragraphs, web-searches for verifiable URLs, edits in place, and reports back URL counts before/after.
