...
> **Backup / tracking note:** most `db/*.db` are git-ignored; `db/notes.db` is force-tracked (`!db/notes.db`). **`db/zsxq.db` (~124 MB) is tracked via Git LFS** (see `.gitattributes`: `db/zsxq.db filter=lfs …`) because it exceeds GitHub's 100 MB plain-git file limit — git stores a ~134-byte pointer, the bytes live in LFS. After a fresh clone, run `git lfs pull` to materialize it. Heads-up on the **LFS free tier (1 GB storage / 1 GB-month bandwidth)**: each DB refresh adds ~124 MB of versioned storage, so ~8 refreshes fill the free quota — prune old versions or add a $5/mo data pack when it fills. An **optional off-GitHub backup** also exists via [`scripts/backup_dbs_to_gcs.sh`](scripts/backup_dbs_to_gcs.sh) (SQLite Online-Backup API → integrity-check → gzip ~67 MB → GCS, read-only on the live DB; set `FINAGENT_GCS_BUCKET=gs://<bucket>`). A **weekly launchd agent** ([`scripts/weekly_db_commit.sh`](scripts/weekly_db_commit.sh), installed from [`scripts/launchd/com.finagent.weekly-db-commit.plist`](scripts/launchd/com.finagent.weekly-db-commit.plist) via `launchctl load -w`) auto-commits `db/zsxq.db` + `db/notes.db` every Sunday 18:00 — only when changed, integrity-checked first, only on `main`, push non-fatal. Logs to `log/weekly_db_commit.log`. NB: at weekly cadence each changed `zsxq.db` adds ~124 MB to LFS storage, so the 1 GB LFS free tier fills in ~8 weeks — add a $5/mo data pack, lengthen the cadence, or route the frequent versioning to GCS.
