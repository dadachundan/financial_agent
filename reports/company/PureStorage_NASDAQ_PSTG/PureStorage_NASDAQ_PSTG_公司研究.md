...
```mermaid
xychart-beta
  title "Everpure (PSTG) 营收与订阅 ARR — FY2024–FY2026 (USD bn)"
...
```mermaid
timeline
  title Everpure (Pure Storage) 关键里程碑
  2009 : 在 Sutter Hill Ventures 办公室创立 (OS76, Inc.) ; 创始人 John Colgrove + John Hayes
  2011-2012 : 推出第一代 FlashArray FA-300 系列 ; 早期获 Greylock / Index / Redpoint 投资
  2015-10 : NYSE 上市 (IPO 价 USD 17, 募资 USD 425 M, 共 25 M 股)
  2017 : Charlie Giancarlo (前 Cisco CTO / 前 Silver Lake) 接任 CEO ; 公司当年实现首个非 GAAP 盈利年度
  2017 : 推出 FlashBlade 平台 ; 进入非结构化/分析型数据存储市场
  2018 : 推出 Evergreen 订阅模式雏形 ; 收购 StorReduce (USD 25 M, 数据去重)
  2019 : 收购 Compuverde (软件定义文件存储)
  2020 : 推出 Evergreen//One 存储即服务 (STaaS) ; 收购 Portworx (USD 370 M, Kubernetes 容器存储)
  2021 : John Colgrove 转任 Chief Visionary Officer ; Ajay Singh 任 Chief Product Officer
  2024-Q3 : 与某顶级 hyperscaler 签订首个"全闪存替代 HDD"设计中标 (公开口径 "major hyperscaler", 媒体广泛指向 Meta)
  2025-03 : 推出 FlashBlade//EXA (面向 GPU 云与 HPC 的并行存储)
  2025-06 : Tarek Robbiati (前 HPE EVP/CFO) 接任 CFO ; Kevan Krysler 转顾问
  2025-11 : Patrick Finn (前 Cloudflare Americas VP) 任 Chief Revenue Officer
  2026-02-23 : 更名为 Everpure, Inc. ; 宣布拟收购 1touch (数据智能/编排)
  2026-03-05 : NYSE 以 Everpure 名义交易 (ticker 仍为 PSTG)
  2026-03-25 : 提交 FY2026 (截止 2026-02-01) 首份以 Everpure 名义的 10-K
```
...
```mermaid
graph LR
    A[Purity OS<br/>统一操作系统] --> B[DirectFlash 模块<br/>raw NAND 直管]
    B --> C[FlashArray<br/>块存储/数据库]
    B --> D[FlashBlade<br/>文件+对象/AI/HPC]
    C --> E[Evergreen 订阅<br/>持续升级]
    D --> E
    E --> F[Evergreen//One<br/>STaaS 按用付费]
    A --> G[Pure Fusion<br/>统一控制平面]
    G --> H[Portworx<br/>K8s/容器存储]
    G --> I[Everpure Cloud<br/>Azure/AWS]
    F --> I
    H --> I
```
...
```mermaid
pie title FY2026 合并营收按客户类别(分析师估算口径, not 10-K 披露)
    "大型企业 (Fortune 500 + Global 2000)" : 70
    "中型企业与公共部门" : 14
    "Hyperscaler 设计中标 (DirectFlash IP)" : 4
    "MSP / 云服务商" : 8
    "联邦政府 / 国防" : 4
```
...
```mermaid
quadrantChart
    title "企业存储竞争定位 (analyst view)"
    x-axis "技术架构旧 -->  全闪存原生"
    y-axis "传统 CapEx -->  订阅/STaaS"
    quadrant-1 "新兴 + 订阅"
    quadrant-2 "老牌 + 订阅"
    quadrant-3 "老牌 + CapEx"
    quadrant-4 "新兴 + CapEx"
    Dell PowerStore: [0.40, 0.30]
    NetApp ONTAP: [0.45, 0.55]
    HPE Alletra: [0.50, 0.45]
    IBM FlashSystem: [0.45, 0.25]
