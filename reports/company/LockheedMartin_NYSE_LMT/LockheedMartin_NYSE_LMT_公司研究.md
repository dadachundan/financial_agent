...
```mermaid
xychart-beta
    title "LMT 营收与 FCF 趋势 (2023-2025, $bn)"
...
```mermaid
xychart-beta
    title "LMT 四大板块营收占比 (FY2025, $M)"
    x-axis [Aeronautics, RMS, MFC, Space]
    y-axis "营收 ($M)" 0 --> 32000
    bar [30257, 17312, 14450, 13029]
```
...
```mermaid
xychart-beta
    title "国防 primes forward P/E 对比 (2026-06-09)"
    x-axis [LMT, NOC, GD, RTX, BA]
    y-axis "forward P/E (x)" 0 --> 55
    bar [16.2, 17.9, 18.8, 23.6, 51.6]
```
...
```mermaid
timeline
    title Lockheed Martin 关键里程碑
    1912 : Loughead (Lockheed) 与 Glenn L. Martin 分别创立前身公司
    1995 : Lockheed Corp + Martin Marietta 合并成立 Lockheed Martin
    2000s : 赢得 Joint Strike Fighter (F-35) 项目
    2020 : Jim Taiclet 出任 CEO (6 月)
    2021 : Taiclet 兼任 Chairman (3 月)
    2026-04 : Orion 完成 Artemis II 载人绕月任务并溅落
    2025-26 : Golden Dome 导弹防御计划启动，LMT 入选太空拦截器承包商
```
...
graph TD
    ...
    LMT --> A["Aeronautics 航空<br/>$30.3bn"]
    LMT --> M["MFC 导弹与火控<br/>$14.5bn"]
    LMT --> R["RMS 旋翼与任务系统<br/>$17.3bn"]
    LMT --> S["Space 太空<br/>$13.0bn ★主题核心"]
    A --> A1["F-35 Lightning II<br/>占总营收 27%"]
    A --> A2["C-130 / F-16 / F-22"]
    A --> A3["Skunk Works ADP<br/>hypersonics / 下一代制空"]
    M --> M1["PAC-3 / THAAD<br/>导弹防御"]
    M --> M2["HIMARS / MLRS / PrSM / JASSM / LRASM"]
    M --> M3["Javelin / Hellfire / Hypersonics"]
    R --> R1["Sikorsky 直升机<br/>Black Hawk / CH-53K"]
    R --> R2["Aegis 作战系统 / 雷达 / C6ISR"]
    S --> S1["Trident II D5 FBM 舰载弹道导弹"]
    S --> S2["Next-Gen OPIR 导弹预警"]
    S --> S3["NGI 下一代拦截器 / GPS III"]
    S --> S4["Orion 载人飞船 (Artemis)"]
    S --> S5["Transport & Tracking Layer 卫星星座"]
```
...
```mermaid
pie title FY2025 营收按客户类型（占合并总营收 $75.0bn）
    "美国政府 - DoW 国防部 (63%)" : 63
    "美国政府 - 其他 (9%)" : 9
    "国际客户含 FMS (28%)" : 28
```
...
```mermaid
xychart-beta
    title "LMT 在手订单 backlog 趋势 ($bn, 年末)"
    x-axis [2023, 2024, 2025]
    y-axis "backlog ($bn)" 0 --> 210
    bar [160.6, 176.0, 193.6]
```
...
```mermaid
quadrantChart
    title 国防 prime 竞争定位（规模 vs 太空/导弹防御敞口）
    x-axis "规模/营收 低" --> "规模/营收 高"
    y-axis "太空/导弹防御敞口 低" --> "太空/导弹防御敞口 高"
    quadrant-1 "全栈巨头"
    quadrant-2 "太空专精"
    quadrant-3 "利基"
    quadrant-4 "陆海规模"
    "Lockheed Martin": [0.90, 0.78]
    "Northrop Grumman": [0.62, 0.85]
    "RTX": [0.85, 0.55]
    "General Dynamics": [0.70, 0.30]
    "Boeing Defense": [0.72, 0.50]
    "Anduril": [0.20, 0.62]
```
...
```mermaid
xychart-beta
    title "全球太空经济 TAM ($bn, source-chained)"
    x-axis [2023, 2030E, 2035E]
    y-axis "市场规模 ($bn)" 0 --> 2000
    bar [630, 1100, 1800]
```
