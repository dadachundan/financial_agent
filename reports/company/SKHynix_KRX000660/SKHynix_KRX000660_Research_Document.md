...
```mermaid
timeline
        title SK hynix — 40-year history (1983–2026)
        1983 : Founded as Hyundai Electronics Industries (Chung Ju-yung)
        1985 : First DRAM — 64-Kbit
        1999 : Hyundai acquires LG Semicon (post-IMF restructuring)
        2001 : Renamed Hynix Semiconductor (creditor control)
        2012 : SK Group acquires controlling stake for KRW 3.4 T — renamed SK hynix
        2013 : Wuxi DRAM fab fire — recovery in 4 months
        2020 : Announces USD 9 bn Intel NAND deal (later Solidigm)
        2022 : Phase-1 close of Intel NAND; HBM3 first-to-market
        2024 : HBM3E 8-Hi to Nvidia; HBM3E 12-Hi mass production
        2025 : HBM4 first to industry, mass-prod prep complete
        2026 : Q1 record KRW 52.6 T revenue; HBM/DRAM sold out 3 years
```
...
```mermaid
graph TD
        A[SK hynix product portfolio] --> B[DRAM ~76% of FY25 revenue]
        A --> C[NAND ~24% of FY25 revenue]
        B --> B1[HBM — HBM3E 8/12-Hi, HBM4]
        B --> B2[Server DRAM — DDR5 RDIMM 64–256GB]
        B --> B3[Mobile DRAM — LPDDR5X / LPDDR5T 16–24GB]
        B --> B4[Graphics DRAM — GDDR6/GDDR7]
        B --> B5[Consumer / PC DRAM — DDR5 UDIMM/SODIMM]
        C --> C1[Enterprise SSD — Solidigm D7/D5 PCIe Gen5]
        C --> C2[Mobile NAND — UFS 4.0 / 4.1]
        C --> C3[Client SSD — PCIe Gen4/Gen5]
        C --> C4[QLC NAND — 321-layer]
```
...
```mermaid
pie title FY2025 revenue mix by end-customer (est. from disclosures + TrendForce)
        "Nvidia" : 30
        "Top-5 hyperscalers (AWS, Azure, Google, Meta, Oracle)" : 22
        "Samsung Electronics (set business)" : 8
        "Apple" : 6
        "AMD" : 5
        "Other OEMs & distribution" : 29
```
...
```mermaid
quadrantChart
        title Memory competitive positioning — HBM (May 2026)
        x-axis "Lower share / breadth" --> "Higher share / breadth"
        y-axis "Behind on tech" --> "Ahead on tech"
        quadrant-1 "Leader"
        quadrant-2 "Tech-first niche"
        quadrant-3 "Lagger"
        quadrant-4 "Scale-first follower"
        "SK hynix": [0.85, 0.85]
        "Samsung": [0.55, 0.4]
        "Micron": [0.35, 0.65]
        "CXMT (China)": [0.2, 0.2]
        "Kioxia (NAND only)": [0.1, 0.5]
```
