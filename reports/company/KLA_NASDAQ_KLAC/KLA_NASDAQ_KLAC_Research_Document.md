...
![KLA — Revenue & Gross Margin, FY2021–FY2025](charts/klac_revenue_gm.png)
...
```mermaid
timeline
    title KLA — Key Milestones
    1975 : KLA Instruments founded by Ken Levy and Robert Anderson; KLA-100 reticle inspection system
    1976 : Tencor Instruments founded
    1992 : First KLA–Tencor merger agreement (broken off 1993)
    1997 : KLA–Tencor merger closed (one-to-one stock swap, $1.3B); Ken Levy steps down as CEO
    2005 : Rick Wallace appointed President & CEO
    2014 : Orbotech (independent) acquires SPTS Technologies for $371M
    2019-02 : KLA closes Orbotech acquisition (~$3.4B), gaining SPTS, PCB, advanced packaging lines
    2019-07 : KLA-Tencor renames to KLA Corporation; "Keep Looking Ahead" identity
    2024-03 : Decision to exit flat-panel Display product manufacturing
    2025-06 : Backlog $7.86B (vs $9.83B prior year as lead times normalize)
    2026-03 : Investor Day — 2030 model: $26B ±$2.5B revenue, 45–47% op margin, $84 ±$8 non-GAAP EPS
    2026-05 : 17th consecutive annual dividend raise; $7B buyback authorization; 10-for-1 stock split announced
```
...
graph TD
    KLA[KLA Corporation]
    KLA --> SPC[Semiconductor Process Control<br/>~90% of revenue]
    KLA --> SSP[Specialty Semiconductor Process<br/>~5%]
    KLA --> PCB[PCB & Component Inspection<br/>~5%]

    SPC --> WI[Wafer Inspection<br/>51% of revenue]
    SPC --> PAT[Patterning / Reticle Inspection<br/>18%]
    SPC --> SVC[Services<br/>22%]
    SPC --> SW[Software & Other<br/>~2%]

    WI --> BFI[Brightfield optical<br/>29xx / 39xx series]
    WI --> DFI[Darkfield optical<br/>Puma / Voyager / 8 series]
    WI --> EBI[E-beam inspection<br/>eDR7380 / eDRX series]
    WI --> METRO[Optical metrology<br/>Archer / SpectraShape / SpectraFilm]

    PAT --> MBMS[EUV/optical reticle inspection<br/>Teron SL6xx / TeraScan 5xx]
    PAT --> CHEM[Chemistry process control<br/>QualiSurf / Quali-Line]

    SSP --> ETCH[SPTS Etch & Deposition<br/>Omega / Sigma / Delta / Primaxx / Xactix]

    PCB --> DI[Direct imaging / inspection<br/>Orbotech Diamond / Nuvogo / Ultra Fusion]
    PCB --> IC_SUB[IC substrate / package inspection<br/>ICOS Tx / Zeta 5xx]
```
...
```mermaid
graph LR
    LITHO[Lithography] --> ETCH[Etch / Deposition]
    ETCH --> CMP[CMP / Clean]
    CMP --> INSP[KLA Inspection<br/>brightfield / darkfield / e-beam]
    INSP --> METRO[KLA Metrology<br/>Archer / SpectraShape / SpectraFilm]
    METRO --> SW[KLA Software<br/>Klarity / 5D Analyzer / aiSIGHT]
    SW --> LITHO
    SW --> ETCH
    SW --> CMP
```
...
```mermaid
pie title KLA FY2025 — Approximate Customer Concentration
    "TSMC (~19% disclosed)" : 19
    "Samsung (>10% disclosed)" : 13
    "Intel / SK Hynix / Micron / leading Chinese foundries (est.)" : 28
    "All other (smaller foundries, IDMs, specialty, packaging, PCB)" : 40
```
...
![KLA — Revenue by Region (ship-to), FY2023–FY2025](charts/klac_geo_mix.png)
...
![KLA — Cash Generation & Capital Return](charts/klac_capital_return.png)
...
```mermaid
quadrantChart
    title Process control / WFE competitive positioning (analyst view)
    x-axis "Narrow product scope" --> "Broad product scope"
    y-axis "Tool-level / hardware" --> "Software + integration"
    quadrant-1 Broad + Integrated
    quadrant-2 Narrow + Integrated
    quadrant-3 Narrow + Hardware
...
![Process-Control / WFE Peers — TTM P/E and P/S (2026-05-20)](charts/klac_peer_multiples.png)
...
```mermaid
graph LR
    SEMI["Global semiconductors<br/>~$11% CAGR 2025-2030"] --> WFE["Wafer fab equipment<br/>~$215B ±$20B by 2030<br/>~12% CAGR"]
    WFE --> PC["Process control / inspection-metrology<br/>~$25-30B by 2030<br/>analyst estimate, ~13-15% CAGR"]
    PC --> KLA["KLA revenue<br/>$26B ±$2.5B target<br/>~13-17% CAGR FY25-2030"]
```
