...
```mermaid
timeline
    title Oracle milestones, 1977–2026
    1977 : Founded by Larry Ellison, Bob Miner, Ed Oates
    1979 : Ships Oracle V2 — first commercial relational DB
    1986 : IPO on NASDAQ at $15/share
    2005 : Acquires PeopleSoft for $10.3B — applications pivot
    2010 : Acquires Sun Microsystems for $7.4B — Java + Solaris + MySQL
    2014 : Safra Catz and Mark Hurd named co-CEOs; Ellison becomes CTO/Chair
    2016 : Acquires NetSuite for $9.3B — cloud-ERP foothold
    2018 : OCI "Gen2" launches under Don Johnson / Clay Magouyrk
    2022 : Closes Cerner acquisition ($28.3B) — Oracle Health
    2024 : OpenAI / Microsoft / Oracle three-way Azure-OCI deal
    2025 : Stargate JV with OpenAI + SoftBank ($500B commitment, $300B contract)
    2025 : RPO surges 359% YoY to $455B (Q1 FY26)
    2025 : Catz steps down; Magouyrk + Sicilia named co-CEOs
    2026 : RPO reaches $553B; FY27 guide raised to $90B
```
...
graph TD
    ...
    ORCL --> CLS[Cloud services + license support<br/>$44.0B FY25]
    ORCL --> CLO[Cloud license + on-prem license<br/>$5.2B FY25]
    ORCL --> HW[Hardware<br/>$2.9B FY25]
    ORCL --> SVC[Services<br/>$5.2B FY25]
    CLS --> APP[Applications cloud + LS<br/>$19.4B]
    CLS --> INF[Infrastructure cloud + LS<br/>$24.6B]
    APP --> FUS[Fusion ERP/HCM/SCM/CX]
    APP --> NS[NetSuite]
    APP --> IND[Industries: Oracle Health, FS, Comms, Retail, Hospitality]
    INF --> OCI[OCI IaaS: compute / GPU / storage]
    INF --> LS[Database + Java + MySQL license support]
    INF --> AUTO[Autonomous Database + Oracle AI Database]
    INF --> MC[Multicloud: OCI@Azure / AWS / GCP]
    HW --> EXA[Exadata]
    HW --> ZFS[ZFS Storage]
    SVC --> CON[Consulting + advanced support]
```
...
```mermaid
pie title Estimated Q3 FY26 RPO concentration by counterparty type (USD B)
    "OpenAI (Stargate, est. 5yr)" : 300
    "Meta (est.)" : 20
    "Other AI / hyperscale (NVIDIA, xAI, TikTok, AMD, etc.)" : 90
    "Traditional enterprise + multicloud DB + SaaS apps" : 143
```
...
```mermaid
quadrantChart
    title Cloud infrastructure positioning — May 2026
    x-axis "Narrow stack" --> "Full enterprise stack"
    y-axis "Slow growth" --> "Fast growth"
    quadrant-1 "Hyperscale stack leaders"
    quadrant-2 "Pure-play AI capacity"
    quadrant-3 "Legacy point tools"
    quadrant-4 "Stack vendors at scale"
    AWS: [0.55, 0.45]
    Azure: [0.70, 0.55]
    GCP: [0.50, 0.65]
    Oracle: [0.85, 0.80]
