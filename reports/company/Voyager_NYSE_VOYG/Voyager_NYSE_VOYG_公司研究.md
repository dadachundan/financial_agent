...
```mermaid
xychart-beta
    title "Voyager 分部净销售额 Net Sales by Segment (USD mn)"
    x-axis ["Defense&NatSec FY24", "Defense&NatSec FY25", "Space Solutions FY24", "Space Solutions FY25", "Starlab FY24", "Starlab FY25"]
    y-axis "USD mn" 0 --> 130
    bar [77.5, 123.0, 74.6, 47.6, 0, 0]
```
...
```mermaid
timeline
    title Voyager Technologies 发展里程碑
    1993 : ZIN Technologies 开发 SAMS（后被收购）
    2019 : Dylan Taylor 创办 Voyager Space Holdings
    2021 : NASA 授予 Nanoracks Starlab 主合同（后转入 Voyager）
    2022 : 为 NASA DART 撞击小行星任务提供动力推进单元
    2023 : 收购 ZIN Technologies；Bishop Airlock 在 ISS 运营
    2025-02 : 更名为 Voyager Technologies, Inc.
    2025-06 : NYSE IPO，发行价 $31.00，募资净额 $409.4M
    2025-08~11 : 收购 EMSI、ExoTerra、Estes Energetics；发行 $460M 可转债
    2026-02 : Starlab 通过 NASA Commercial 关键设计评审（CCDR）
```
...
```mermaid
graph TD
    V[Voyager Technologies] --> D[Defense & National Security<br/>FY25 营收 $123.0M]
    V --> S[Space Solutions<br/>FY25 营收 $47.6M]
    V --> SL[Starlab Space Stations<br/>FY25 营收 $0 · 期权]
    D --> D1[Defense Systems<br/>可控固体推进 + 含能材料 Estes]
    D --> D2[Signals Intelligence<br/>信号情报 + 电子战 EMSI]
    D --> D3[Communications<br/>抗辐射激光/RF 通信]
    D --> D4[Guidance Nav & Control<br/>星敏感器/惯性测量 NGI]
    D --> D5[AI Edge Computing<br/>抗辐射 GPU + Palantir]
    D --> D6[Space Maneuver<br/>电/化学推进 ExoTerra]
    S --> S1[Advanced Space Tech<br/>在轨推进/制造]
    S --> S2[Space Infrastructure<br/>Bishop Airlock]
    S --> S3[Science & Mission Mgmt<br/>Nanoracks · ISS 服务]
    SL --> SL1[Starlab 商业空间站<br/>NASA CLD · 2029 发射]
```
...
```mermaid
pie title FY2025 合并营收按客户（consolidated revenue）
    "U.S. Government（美国政府，含通过中间商）" : 86
    "商业及其他（commercial & other）" : 14
```
...
```mermaid
quadrantChart
    title 太空/国防标的竞争定位（分析师视角，仅示意）
    x-axis "纯部件供应 Component" --> "全栈/平台 Full-stack"
    y-axis "商业叙事溢价低" --> "商业叙事溢价高"
    quadrant-1 "高溢价全栈"
    quadrant-2 "高溢价专精"
    quadrant-3 "低溢价专精"
    quadrant-4 "低溢价全栈"
    "Voyager (VOYG)": [0.55, 0.62]
    "Rocket Lab (RKLB)": [0.85, 0.92]
    "Redwire (RDW)": [0.40, 0.45]
    "Northrop Grumman": [0.90, 0.30]
```
...
```mermaid
xychart-beta
    title "全球太空经济规模 Space Economy (USD trillion)"
    x-axis ["2023", "2035E (base)"]
    y-axis "USD trillion" 0 --> 2
    bar [0.63, 1.8]
```
