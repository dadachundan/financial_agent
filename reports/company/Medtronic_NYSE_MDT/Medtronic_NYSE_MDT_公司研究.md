...
```mermaid
xychart-beta
    title "MDT 营收与非 GAAP 经营利润率趋势（FY25–FY27E）"
    x-axis ["FY25A", "FY26A", "FY27E"]
    y-axis "营收 (US$ bn)" 0 --> 40
    bar [33.5, 36.4, 38.9]
    line [25.7, 24.4, 24.0]
```
...
```mermaid
timeline
    title Medtronic 关键里程碑
    1949 : 明尼阿波利斯车库创立（Bakken & Hermundslie）
    1957 : 发明首台可穿戴电池起搏器
    1960 : 确立使命 "alleviate pain, restore health, and extend life"
    2015 : 以约 $499 亿收购 Covidien，完成 tax inversion，转为爱尔兰注册的 Medtronic plc
    2020 : Geoff Martha 出任 CEO
    2021 : Hugo RAS 获 CE Mark（欧洲），开启 ex-US 商业化
    2023-24 : 重组为四大组合（Cardiovascular/Neuroscience/Medical Surgical/Diabetes）；退出呼吸机产品线
    2025 : 5 月宣布分拆 Diabetes；12-03 Hugo 获美国 urology 批准
    2026 : MiniMed（Diabetes）在 Nasdaq 上市（代码 MMED，3-06）；FY26 录得十年最强营收增长
```
...
```mermaid
graph TD
    MDT[Medtronic plc<br/>FY26 营收 $36.4bn] --> CV[Cardiovascular $14.0bn<br/>最大且最快]
    MDT --> NS[Neuroscience $10.3bn]
    MDT --> MS[Medical Surgical $8.8bn]
    MDT --> DB[Diabetes/MiniMed $3.1bn<br/>分拆中]
    CV --> CRHF[Cardiac Rhythm & Heart Failure<br/>Micra/Affera/PFA]
    CV --> SHA[Structural Heart & Aortic<br/>CoreValve Evolut TAVR]
    CV --> CPV[Coronary & Peripheral<br/>支架/Symplicity RDN]
    NS --> CST[Cranial & Spinal<br/>Mazor 机器人/StealthStation]
    NS --> ST[Specialty Therapies]
    NS --> NM[Neuromodulation<br/>DBS/SCS]
    MS --> SE[Surgical & Endoscopy<br/>吻合器/能量/**Hugo RAS**]
    MS --> ACM[Acute Care & Monitoring]
    DB --> DOU[MiniMed 780G AID + Simplera CGM]
```
...
```mermaid
pie title MDT FY26 营收按组合（consolidated revenue 口径）
    "Cardiovascular" : 38.4
    "Neuroscience" : 28.3
    "Medical Surgical" : 24.2
    "Diabetes/MiniMed" : 8.6
```
...
```mermaid
xychart-beta
    title "手术机器人全球收入池增长（US$ bn）"
    x-axis ["2025", "2030E", "2035E"]
...
```mermaid
quadrantChart
    title 手术机器人竞争定位（装机规模 vs 软组织适应症广度）
    x-axis "装机规模小" --> "装机规模大"
    y-axis "适应症窄" --> "适应症广"
    quadrant-1 "在位领导者"
    quadrant-2 "广度挑战者"
    quadrant-3 "利基/早期"
    quadrant-4 "规模但窄"
    "Intuitive da Vinci": [0.92, 0.85]
    "Medtronic Hugo": [0.35, 0.55]
    "J&J Ottava": [0.15, 0.45]
    "MedBot 图迈": [0.25, 0.40]
    "Procept (PRCT)": [0.20, 0.18]
```
