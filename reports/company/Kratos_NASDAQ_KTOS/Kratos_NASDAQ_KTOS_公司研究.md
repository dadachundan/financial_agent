...
```mermaid
xychart-beta
    title "Kratos 营收（$M，柱）与 gross margin（%，线）FY2022–FY2025"
    x-axis [FY2022, FY2023, FY2024, FY2025]
    y-axis "营收 $M / 毛利率 %" 0 --> 1400
    bar [898.3, 1037.1, 1136.3, 1346.8]
    line [251, 259, 253, 229]
```
...
timeline
    title Kratos 关键里程碑
    1994/1995 : 纽约州注册（前身无线通信公司）/ 开始运营
    1998 : 迁册特拉华州
    2003/2004 : Eric DeMarco 加入任总裁兼COO / 2004-04 出任CEO，启动国防科技转型
    2021 : OpenSpace 虚拟化卫星地面系统 first-to-market 发布
    2024 : Erinyes 高超声速首飞（6月）/ Zeus SRM 首次成功飞行（10月）
    2025 : Erinyes 二飞 / 中标史上最大单 MACH-TB 2.0（~$1.45B）/ 还清债务实现净现金 / 与GE Aerospace 签GEK800
    2026 : Q1 营收+22.6%、book-to-bill 1.6x、backlog 突破 $2.0B / 上调FY26指引 / 完成 Nomad、Orbit 收购
```
...
```mermaid
graph TD
    K[Kratos KTOS] --> KGS[KGS 政府解决方案]
    K --> US[Unmanned Systems 无人系统]
    KGS --> MW[Microwave Electronics 微波电子]
    KGS --> SP[Space/Satellite/Cyber 航天卫星网络]
    KGS --> TT[Turbine Technologies 涡轮推进/喷气发动机]
    KGS --> RK[Rocket/Hypersonic 火箭与高超声速]
    KGS --> C5[C5ISR/Modular 模块化系统]
    KGS --> TR[Training 训练仿真]
    US --> UA[Unmanned Aerial 无人机]
    US --> UG[Unmanned Ground 无人地面]
    US --> USB[Unmanned Seaborne 无人海上]
    UA --> VAL[Valkyrie XQ-58A 协同作战飞机]
    UA --> TGT[靶机 BQM-167/BQM-177/MQM-178/Firejet]
    RK --> ZEUS[Zeus 固体火箭发动机 SRM / Oriole]
    RK --> ERI[Erinyes / Dark Fury 高超声速飞行器]
    SP --> OS[OpenSpace 虚拟化卫星地面系统]
```
...
```mermaid
pie title FY2025 营收结构（按客户类型，合并口径）
    "美国政府含FMS ~68%" : 68
    "大型集成商/商业/国际 ~32%" : 32
```
...
```mermaid
quadrantChart
    title 国防科技竞争定位（规模 × 颠覆性）
    x-axis "资源/规模小" --> "资源/规模大"
    y-axis "迭代慢/传统" --> "颠覆快/低成本"
    quadrant-1 "大而快（理想区）"
    quadrant-2 "小而颠覆"
    quadrant-3 "小而传统"
    quadrant-4 "大而传统"
    "Kratos": [0.42, 0.78]
    "Anduril": [0.40, 0.92]
    "Lockheed/Northrop": [0.92, 0.30]
    "AeroVironment": [0.38, 0.62]
    "General Atomics": [0.62, 0.55]
    "Mercury Systems": [0.35, 0.40]
```
...
```mermaid
xychart-beta
    title "Kratos 四大可寻址市场规模（$B，2025E vs 2030E/2035E）"
    x-axis ["军用无人机", "高超声速武器", "Golden Dome年度", "空间经济(/100)"]
    y-axis "$B" 0 --> 25
    bar [15.8, 8.24, 13.4, 6.3]
    bar [22.81, 14.78, 13.4, 18.0]
```
