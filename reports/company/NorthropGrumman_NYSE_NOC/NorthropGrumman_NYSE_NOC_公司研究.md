...
```mermaid
xychart-beta
    title "NOC 收入与自由现金流 (FCF) 走势, FY2021–FY2025 ($bn)"
    x-axis [FY2021, FY2022, FY2023, FY2024, FY2025]
    y-axis "金额 ($bn)" 0 --> 45
    bar [35.67, 36.60, 39.29, 41.03, 41.95]
    line [2.97, 2.02, 2.10, 2.62, 3.31]
```
...
```mermaid
timeline
    title Northrop Grumman 关键里程碑
    1939 : Northrop Aircraft 在加州 Hawthorne 成立
    1994 : 收购 Grumman Corporation，更名为 Northrop Grumman
    1996 : 收购 Westinghouse 国防与电子业务（雷达/电子系统）
    2001 : 收购 Litton Industries 与 Newport News Shipbuilding（电子/造船）
    2002 : 收购 TRW Inc.（军用与民用太空系统、载荷）
    2011 : 剥离 Huntington Ingalls Industries（造船业务分拆上市）
    2018 : 收购 Orbital ATK（卫星、运载火箭、固体火箭发动机/导弹）
    2021 : 向 Veritas Capital 出售 IT 与任务支持服务业务
```
...
```mermaid
xychart-beta
    title "NOC FY2025 四段收入 ($bn) — Space Systems 高亮 (25.7%)"
    x-axis ["Aeronautics(AS)", "Mission(MS)", "Space(SS)*", "Defense(DS)"]
    y-axis "段收入 ($bn)" 0 --> 14
    bar [12.99, 12.51, 10.77, 8.00]
```
...
```mermaid
graph TD
    A[Northrop Grumman] --> B[Aeronautics Systems<br/>平台: B-21/E-2D/Triton]
    A --> C[Defense Systems<br/>火力/威慑: Sentinel ICBM/IBCS/导弹/SRM]
    A --> D[Mission Systems<br/>探测/指控: 雷达/EW/C4ISR/微电子]
    A --> E[Space Systems<br/>天基: 卫星/跟踪层/拦截器/推进]
    D -->|传感器与指控馈入| C
    D -->|火控雷达/航电装机| B
    E -->|导弹预警/跟踪数据| C
    C -->|SRM 推进共用| E
    E -->|天基拦截器| F[Golden Dome 导弹防御全栈]
    C -->|陆基拦截/IBCS| F
    D -->|战场管理网络| F
```
...
```mermaid
pie title NOC FY2025 收入按客户类型 (占合并总收入)
    "美国政府 (U.S. Government)" : 84
    "国际客户与商用 (International & Commercial)" : 16
```
...
```mermaid
quadrantChart
    title 主承包商定位 (太空敞口 vs 核威慑/平台护城河)
    x-axis "太空敞口低" --> "太空敞口高"
    y-axis "护城河窄" --> "护城河宽 (核威慑/隐身平台)"
    quadrant-1 "高太空+宽护城河"
    quadrant-2 "低太空+宽护城河"
    quadrant-3 "低太空+窄护城河"
    quadrant-4 "高太空+窄护城河"
    Northrop Grumman: [0.82, 0.88]
    Lockheed Martin: [0.62, 0.80]
    RTX: [0.45, 0.55]
    General Dynamics: [0.30, 0.50]
    L3Harris: [0.55, 0.40]
    Boeing: [0.50, 0.45]
```
...
```mermaid
xychart-beta
    title "全球太空经济 TAM 增长 ($bn, WEF/McKinsey)"
    x-axis [2023, 2035E]
    y-axis "$bn" 0 --> 2000
    bar [630, 1800]
```
