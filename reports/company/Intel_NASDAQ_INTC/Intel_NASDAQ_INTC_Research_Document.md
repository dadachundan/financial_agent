...
```mermaid
timeline
    title Intel Corporation — Key Milestones (1968–2026)
    1968 : Founded by Gordon Moore and Robert Noyce in Mountain View, CA
    1971 : Ships Intel 4004 — world's first commercial microprocessor
    1985 : Exits DRAM business; pivots to microprocessors under Andrew Grove
    1991 : "Intel Inside" co-marketing program launched
    1993 : Pentium processor launched; brand becomes mass-market household name
    1999 : Joins Dow Jones Industrial Average
    2008 : Atom processor for low-power devices; misses mobile (smartphone) wave
    2017 : Mobileye acquired for USD 15.3B
    2021 : Pat Gelsinger returns as CEO; announces IDM 2.0 strategy and Intel Foundry Services
    2022 : Mobileye spun out via IPO (Intel retains majority stake); peak revenue USD 79.0B in FY2021 rolls into rapid decline
    2024 : Pat Gelsinger steps down December 2024; David Zinsner and Michelle Johnston Holthaus serve as interim co-CEOs
    2025 : Lip-Bu Tan appointed CEO (March); USD 8.9B U.S. government equity investment (August); NVIDIA invests USD 5.0B and SoftBank invests USD 2.0B (September); Altera 51% sold to Silver Lake; Intel 18A enters high-volume manufacturing
    2026 : Intel removed from Dow Jones Industrial Average (Nov 2024); operating GAAP gross margin recovers to 39.4% Q1; Q1 revenue +7% YoY at USD 13.6B
```
...
graph TD
    ...
    INTC --> P[Intel Products]
    INTC --> F[Intel Foundry]
    INTC --> O[All Other]

    P --> CCG[CCG — Client Computing]
    P --> DCAI[DCAI — Data Center & AI]

    CCG --> Core[Intel Core / Core Ultra<br>desktop & mobile]
    CCG --> CoreS[Core Ultra Series 2 & 3<br>Lunar Lake, Arrow Lake, Panther Lake]
    CCG --> vPro[Intel vPro<br>commercial PC platform]
    CCG --> Arc[Intel Arc<br>discrete GPUs]
    CCG --> Wifi[Wi-Fi & Bluetooth<br>connectivity silicon]

    DCAI --> Xeon[Intel Xeon 6<br>P-core & E-core servers]
    DCAI --> Xeon600[Xeon 600<br>workstation processors]
    DCAI --> Gaudi[Gaudi 3<br>AI training accelerator]
    DCAI --> GPU[GPU Max<br>HPC accelerator]
    DCAI --> IPU[IPU / SmartNIC<br>data-center networking]

    F --> N7[Intel 7<br>high-volume HVM]
    F --> N3[Intel 3<br>data-center node]
    F --> N18A[Intel 18A<br>RibbonFET + PowerVia, HVM 2025]
    F --> N14A[Intel 14A<br>next-gen, designed for external]
    F --> AP[Foveros & EMIB<br>advanced packaging]

    O --> Mobileye[Mobileye Global<br>ADAS / EyeQ / SuperVision]
    O --> IMS[IMS Nanofabrication<br>multi-beam mask writers]
    O --> Altera[Altera<br>49% equity-method stake]
```
...
```mermaid
pie title FY2024 disclosed customer concentration (% of net revenue)
    "Dell Technologies" : 19
    "Lenovo Group" : 11
    "HP Inc." : 10
    "All other (incl. hyperscalers + OEMs)" : 60
```
...
```mermaid
quadrantChart
    title Semiconductor competitive positioning — Process leadership × Product breadth
    x-axis "Narrow product portfolio" --> "Broad product portfolio"
    y-axis "Lagging on leading-edge process" --> "Leading-edge process leadership"
    quadrant-1 "Process-led integrators"
    quadrant-2 "Process leaders"
    quadrant-3 "Mature-node players"
    quadrant-4 "Fabless breadth"
    "Intel (IDM 2.0)": [0.78, 0.62]
    "TSMC (foundry)": [0.55, 0.95]
    "AMD (fabless)": [0.62, 0.72]
    "NVIDIA (fabless)": [0.78, 0.78]
    "Samsung Foundry": [0.45, 0.55]
    "Broadcom (fabless)": [0.72, 0.32]
    "Qualcomm (fabless)": [0.58, 0.68]
    "GlobalFoundries": [0.32, 0.18]
```
