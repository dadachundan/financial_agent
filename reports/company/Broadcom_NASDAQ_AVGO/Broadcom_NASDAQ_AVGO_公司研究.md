...
timeline
    ...
    2018 : 高通 (Qualcomm) 敌意收购要约被 CFIUS / 行政命令阻止;重新注册至特拉华州
...
graph TD
    ...
    B --> B1[网络连接 Networking Connectivity]
    B --> B2[无线设备连接 Wireless Device Connectivity]
    B --> B3[服务器与存储 Servers & Storage]
    B --> B4[宽带 Broadband]
    B --> B5[工业 Industrial]
    B1 --> B1a[定制 XPU ASIC - Google TPU、Meta MTIA]
    B1 --> B1b[Tomahawk / Jericho / Trident 以太网交换芯片]
    B1 --> B1c[以太网 NIC / PHY / 光器件]
    B2 --> B2a[FBAR RF 滤波器 / 前端模块]
    B2 --> B2b[Wi-Fi/BT 合并芯片、触控、感应充电]
    B3 --> B3a[PCIe 交换芯片]
    B3 --> B3b[SAS/RAID、光纤通道 HBA]
    B3 --> B3c[HDD SoC + 前置放大器;定制 SSD 控制器]
    B4 --> B4a[机顶盒 SoC]
    B4 --> B4b[宽带接入 — DSL、有线、PON、Wi-Fi]
    B5 --> B5a[光耦合器、运动编码器、工业传感器]
    C --> C1[私有云 — VMware Cloud Foundation、vSphere、Tanzu、Avi、vDefend]
    C --> C2[大型机 — 源自 CA 的 AIOps、DB/DM、DevX、安全、基础类]
    C --> C3[网络安全 — Symantec、Carbon Black]
    C --> C4[企业软件 — AIOps、DevOps、ValueOps]
    C --> C5[FC SAN 管理 — Brocade]
```
...
![Broadcom 分部营业收入结构](charts/avgo_segment_mix.png)
...
![FCF 与债务](charts/avgo_fcf_debt.png)
