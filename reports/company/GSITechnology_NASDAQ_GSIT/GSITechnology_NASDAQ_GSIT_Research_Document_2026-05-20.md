...
![Revenue and gross margin trend](../../charts/gsit_revenue_gm.png)
...
```mermaid
timeline
    title GSI Technology — Major Milestones (1995–2026)
    1995 : Founded as Giga Semiconductor (Sunnyvale, CA)
         : Lee-Lean Shu and team from Sony Microelectronics
    2003 : Renamed GSI Technology
    2004 : Reincorporated in Delaware
    2007 : IPO on NASDAQ (ticker GSIT)
    2009 : Acquired Sony's SRAM product line; gained Sony memory team led by Patrick Chuang
    2015 : Acquired MikaMonu Group (Israel) — origin of APU / associative-computing IP
    2018 : First-generation Gemini-I APU launched
    2019 : Gemini-I first silicon, demos for similarity search
    2023 : Awarded Space Development Agency prototype for APU2 (radhard)
         : Filed S-3 shelf, set up $25M ATM with Needham
    2024 : Q3 FY24 took $2.4M mask-set charge on APU2 / 16nm
         : May — initiated formal strategic review (Needham as advisor)
         : June — sold and leased back Sunnyvale HQ for $11.2M
         : August — 16% workforce reduction, $3.5M annualized cost cut
    2025 : Jan — received Gemini-II first silicon
         : Mar — AFWERX SBIR Phase II ($1.1M) on APU2 edge AI
         : June — FY2025 10-K filed; cash $13.4M; net loss $10.6M
         : Aug — ATM offering raised $14.3M
         : Oct — $50M registered-direct offering closes (price $10/share)
    2026 : Jan — Gemini-II 3s/30W multimodal LLM benchmark disclosed
         : Mar — G2 Tech (Israel) "Sentinel" drone partnership announced
         : May — FY2026 revenue $25.1M (+22%); ~$2M US Army SBIR; smart-city Phase I
```
...
```mermaid
graph TD
    GSI[GSI Technology<br/>NASDAQ:GSIT]
    GSI --> SRAM[Very Fast SRAM<br/>≈100% of revenue]
    GSI --> APU[Gemini APU<br/>compute-in-memory]
    GSI --> SBIR[Government R&D<br/>SBIR/Space Force/Army]

    SRAM --> SQ[SigmaQuad / SigmaDDR<br/>synchronous SRAM]
    SRAM --> LL[LLDRAM / pseudo-DRAM<br/>NoBL/ZBT]
    SRAM --> RH[Radiation-hardened &<br/>radiation-tolerant SRAM<br/>mil/aero/space]
    SRAM --> HD[High-density monolithic<br/>up to 288Mb]

    APU --> G1[Gemini-I<br/>28nm, in production<br/>Leda-E PCIe / Leda-S 1U card]
    APU --> G2[Gemini-II<br/>16nm, sampling 2025<br/>Leda-E2 PCIe card]
    APU --> P[Plato<br/>next-gen edge APU<br/>tape-out targeted Q1 CY2027]
    APU --> APU2[APU2<br/>radhard variant for<br/>Space Development Agency]
    APU --> SaaS[APU-as-a-Service<br/>AWS connected,<br/>OpenSearch vector search]
```
...
```mermaid
pie title FY2025 Net Revenues by End Customer
    "KYEC (test & measurement, Taiwan)" : 23
    "Nokia (networking, Finland)" : 12
    "Cadence Design Systems (chip design)" : 8
    "Flextronics (contract mfg.)" : 3
    "All other OEMs" : 54
```
...
![Customer concentration over time](../../charts/gsit_customer_concentration.png)
...
![SRAM end-market mix](../../charts/gsit_sram_market_mix.png)
...
```mermaid
quadrantChart
    title Competitive Positioning — Edge AI Accelerators / CIM
    x-axis Low Power Efficiency --> High Power Efficiency
    y-axis Low Software Ecosystem --> Mature Software Ecosystem
    quadrant-1 Best in class
    quadrant-2 Mature, power-hungry
    quadrant-3 Niche / immature
    quadrant-4 Differentiated but lonely
    NVIDIA Jetson Thor: [0.30, 0.95]
    Qualcomm Snapdragon X Elite: [0.55, 0.75]
    Apple Neural Engine: [0.80, 0.65]
    GSI Gemini-II: [0.85, 0.20]
    Mythic AMP: [0.70, 0.15]
    Hailo-10: [0.65, 0.35]
    AMD MI300A: [0.20, 0.85]
    Groq LPU: [0.45, 0.45]
```
