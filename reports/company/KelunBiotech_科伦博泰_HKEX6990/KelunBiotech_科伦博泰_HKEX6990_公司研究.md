...
```mermaid
xychart-beta
    title "Kelun-Biotech 营收与毛利率 (RMB million)"
    x-axis ["2022", "2023", "2024", "2025"]
    y-axis "Revenue (RMB million)" 0 --> 2500
    bar [792, 1540, 1933, 2058]
    line [40, 50, 65.9, 71.8]
```
...
```mermaid
timeline
    title Kelun-Biotech 关键里程碑
    1996 : 科伦药业 (Kelun Pharmaceutical) 由刘革新等创立，大输液起家
    2010 : 科伦药业深交所上市 (SZSE:002422)
    2012 : "三发驱动 (输液+抗生素+创新药)" 战略启动
    2016 : 四川科伦博泰生物医药正式成立 — 创新药业务独立载体
    2022.05 : 与 Merck 就 SKB264 (TROP2 ADC) 签订首项 license-out 协议 — 里程碑上限 USD 1.36B
    2022.07 : SKB315 (CLDN18.2 ADC) license-out — 里程碑上限 USD 901M
    2022.12 : 7 款临床前 ADC 打包授权 Merck — 里程碑上限 USD 9.3B
    2023.07.11 : 港交所 IPO 上市 (HKEX:6990)
    2024.08 : Merck 退还 SKB315 权益；同时行使 SKB571 选择权 USD 37.5M
    2024.11 : sac-TMT (SKB264) 获 NMPA 批准上市 — 中国首个本土 TROP2 ADC
    2025.02 : sac-TMT 第二项适应症 (HR+/HER2- BC) 获 NMPA 批准
    2025 : 三款产品 (5 项适应症) 纳入 NRDL；商业化团队扩张至 400+ 人
    2026.02 : sac-TMT NDA 三阴性乳腺癌 1L 等多项适应症推进
```
...
```mermaid
graph TD
    A[科伦博泰核心管线] --> B[ADC / 偶联药物平台 OptiDC]
    A --> C[biologics 生物制品平台]
    A --> D[small molecule 小分子平台]
    B --> B1[sac-TMT 佳泰莱 TROP2 ADC 已上市]
    B --> B2[trastuzumab botidotin 舒泰莱 HER2 ADC 已上市]
    B --> B3[SKB315 CLDN18.2 ADC 已收回权益]
    B --> B4[SKB571 双抗 ADC Merck 已选择]
    B --> B5[7 款临床前 ADC 资产 Merck 全球权益]
    C --> C1[tagitanlimab 科泰莱 PD-L1 mAb 已上市]
    C --> C2[cetuximab N01 达泰莱 EGFR mAb 已上市]
    C --> C3[其他 mAb / BiAb 管线 临床推进中]
    D --> D1[oncology 小分子]
    D --> D2[metabolic 小分子]
```
...
```mermaid
pie title 2025 年收入结构 (RMB million, 合并收入)
    "Licensing & collaboration income (Merck 为主)" : 1515
    "Product sales (中国市场: sac-TMT, tagitanlimab 等)" : 543
```
...
```mermaid
xychart-beta
    title "全球 ADC 市场规模 (USD billion)"
    x-axis ["2022", "2024", "2025", "2030E", "2035E"]
    y-axis "Market size (USD billion)" 0 --> 40
    bar [9, 11, 13, 23, 35]
```
...
quadrantChart
    ...
    Kelun-Biotech 6990: [0.85, 0.85]
    RemeGen 9995: [0.4, 0.6]
    Hengrui 600276: [0.7, 0.45]
    Innovent 1801: [0.6, 0.55]
    BeiGene 6160: [0.5, 0.55]
    WuXi XDC 2268: [0.85, 0.6]
```
...
```mermaid
xychart-beta
    title "中国 ADC 市场 SAM 与 sac-TMT 销售潜力 (RMB billion)"
    x-axis ["2024A", "2025E", "2026E", "2028E", "2030E"]
    y-axis "RMB billion" 0 --> 30
    bar [6, 10, 14, 20, 25]
    line [0.1, 0.9, 2.5, 4, 5]
```
...
graph LR
    ...
    A --> D[C. 财务 / 治理]
    A --> E[D. 宏观 / 监管]
    B --> B1[A1 客户集中度 Merck 70%+]
    B --> B2[A2 license-out partner-discretion]
    B --> B3[A3 临床试验失败]
    B --> B4[A4 商业化执行]
    C --> C1[B1 ADC 同类竞争加剧]
    C --> C2[B2 NRDL 价格谈判压力]
    ...
    D --> D1[C1 持续亏损 + 现金消耗]
    D --> D2[C2 母公司控股 + 关联交易]
    D --> D3[C3 估值倍数压缩 ~50× P/S]
    E --> E1[D1 中美脱钩 / BIOSECURE]
    E --> E2[D2 NMPA 监管节奏]
```
