...
```mermaid
timeline
    title Firefly Aerospace 关键里程碑
    2014 : Firefly Space Systems 创立（Tom Markusic）
    2016-17 : 破产清算 → Noosphere/Polyakov 拍卖购入资产，重组为 Firefly Aerospace
    2022 : AE Industrial Partners 取得控股 ; Eclipse(MLV) 与 Northrop Grumman 合作宣布
    2022-10 : Alpha 首次成功入轨（1,000 kg 级，美国唯一）
    2023-09 : VICTUS NOX 为 USSF 创下 ~24 小时响应发射纪录
    2025-03 : Blue Ghost Mission 1 成功登月（商业公司首次完整月面任务）
    2025-04 : Alpha 发射出现 anomaly，FAA 要求 mishap investigation
    2025-08 : IPO 上市（NASDAQ: FLY，$45/股，~$868M 募资）
    2025-10 : 完成 SciTec 收购（$547M，国防软件 / Golden Dome）
    2026 : Eclipse 首飞建造完成（预期）; Blue Ghost Mission 2
```
...
```mermaid
graph TD
    F[Firefly Aerospace] --> L[Launch Solutions 发射]
    F --> S[Spacecraft Solutions 航天器]
    F --> D[SciTec 国防软件/数据]
    L --> A["Alpha — small launch vehicle / SLV<br/>1,000 kg 级，已 7 次发射 / 5 次成功入轨"]
    L --> E["Eclipse / MLV — medium launch vehicle<br/>16,000 kg 到 LEO，与 Northrop Grumman 合研，研发中"]
    S --> BG["Blue Ghost — lunar lander 月球着陆器<br/>唯一商业登月成功"]
    S --> EL["Elytra — orbital / space utility vehicle<br/>在轨转移/SDA/通信中继"]
    D --> MW["Missile warning & defense / ISR / SDA<br/>支持 Golden Dome"]
    A -.共享发动机/结构/软件.-> E
    BG -.共享 thrusters/avionics/software.-> EL
```
...
```mermaid
pie title FY2025 营收客户集中度（consolidated 口径）
    "Customer 1 (~NASA, 未点名)" : 59.1
    "Customer 2" : 8.9
    "Customer 3" : 3.4
    "其余全部客户" : 28.6
```
...
```mermaid
quadrantChart
    title 发射与航天器竞争定位（运力规模 vs 国防/响应能力）—— 分析师观点
    x-axis "低运力规模" --> "高运力规模"
    y-axis "纯商业导向" --> "国防/响应导向"
    quadrant-1 "国防巨头/规模化"
    quadrant-2 "国防专精"
    quadrant-3 "小众商业"
    quadrant-4 "商业规模化"
