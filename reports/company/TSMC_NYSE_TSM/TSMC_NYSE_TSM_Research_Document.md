...
```mermaid
timeline
    title TSMC Milestones, 1987–2026
    1987 : Founded as JV with R.O.C. govt. & Philips
    1994 : IPO on Taiwan Stock Exchange (TWSE: 2330)
    1997 : ADR listing on NYSE (TSM)
    1999 : 0.18 micron volume production (first major leading-edge node)
    2005 : Morris Chang steps down; Rick Tsai becomes CEO
    2009 : Chang returns as CEO; Mark Liu and C.C. Wei rise
    2011 : 28nm volume production; Apple becomes major customer
    2014 : 20nm volume production (Apple A8)
    2016 : 16nm FinFET ramp; Apple A10
    2018 : Chang retires permanently; Mark Liu Chairman, C.C. Wei CEO
    2020 : 5nm volume production (Apple A14, M1)
    2022 : 3nm volume production begins
    2024 : Arizona Fab 21 begins 4nm production; Kumamoto Fab 23 starts up
    2025 : N2 (2nm) volume production; Mark Liu retires, C.C. Wei becomes Chairman+CEO
    2026 : A16 risk production; Arizona Fab 2 (3nm) accelerated; Dresden ESMC tool move-in
```
...
```mermaid
graph TD
    A[TSMC Process Technologies]
    A --> B[Leading-edge logic]
    A --> C[Mainstream logic]
    A --> D[Specialty]
    A --> E[Advanced packaging<br/>3DFabric]
    B --> B1[A16 1.6nm risk production 2026]
    B --> B2[N2 / N2P 2nm Nanosheet — VP 2025]
    B --> B3[N3 / N3E / N3P / N3X / N3A / N3C — VP since 2022]
    B --> B4[N4 / N4P / N4C — derivative]
    B --> B5[N5 / N5P — VP since 2020]
    B --> B6[N6 / N7 / N7+ FinFET]
    C --> C1[16nm FinFET / 16FFC]
    C --> C2[22ULP / 22ULL]
    C --> C3[28nm HPC / HPC+ / ULP]
    C --> C4[40 / 55 / 65 / 90nm planar]
    C --> C5[0.13 / 0.18 / 0.25um — mature]
    D --> D1[RF / RF front-end]
    D --> D2[Embedded NVM eFlash]
    D --> D3[Power Management ICs]
    D --> D4[CMOS image sensors]
    D --> D5[MRAM / RRAM]
    D --> D6[MEMS]
    E --> E1[CoWoS — CoW + on Substrate]
    E --> E2[InFO — fan-out wafer-level]
    E --> E3[SoIC — chip-on-wafer 3D]
    E --> E4[CoW / WoW die-stacking]
```
...
```mermaid
pie title TSMC FY2025 net revenue — top customers (estimated allocation)
    "NVIDIA (#1, ~19%)" : 19
    "Apple (#2, ~17%)" : 17
    "AMD (~7%)" : 7
    "Qualcomm (~6%)" : 6
    "Broadcom (~5%)" : 5
    "MediaTek (~5%)" : 5
    "Customers 7–10 (~19%)" : 19
    "All other customers (~22%)" : 22
```
