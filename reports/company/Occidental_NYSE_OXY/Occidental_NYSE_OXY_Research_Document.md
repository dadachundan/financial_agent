...
![Segment net sales](charts/oxy_segment_sales.png)
...
```mermaid
timeline
    title Occidental Petroleum — Selected Milestones
    1920 : Founded as a California oil exploration company
    1968 : Acquires Hooker Chemical (precursor to OxyChem)
    1986 : Acquires MidCon — first major U.S. gas storage / midstream footprint
    2005 : Assumes operatorship of Mukhaizna (Block 53) in Oman — large-scale steam EOR
    2016 : Vicki Hollub becomes President and CEO — first female CEO of a major U.S. oil company
    2019 : Closes $55B Anadarko acquisition; Berkshire invests $10B in 8% preferred + warrants at $59.59
    2020 : COVID demand collapse — dividend cut to $0.01/qtr; debt under stress
    2022 : Berkshire receives FERC approval to acquire up to 50% of OXY common
    2023 : Acquires Carbon Engineering for $1.1B; OLCV / 1PointFive scale-up begins
    2024 : Closes $12.4B CrownRock acquisition (Aug-2024); production jumps to 1,327 Mboe/d
    2025 : Announces $9.7B sale of OxyChem to Berkshire Hathaway (Oct-2025)
    2026 : Closes OxyChem sale (Jan-2026); STRATOS DAC begins initial operations (Q2-2026); principal debt to $13.3B
```
...
```mermaid
graph TD
    OXY[Occidental Petroleum] --> OG[Oil & Gas Segment]
    OXY --> MID[Midstream & Marketing Segment]
    OG --> Permian[Permian Basin<br/>786 Mboe/d, $3.4B capex 2025]
    OG --> Rockies[Rockies & Other<br/>DJ Basin, Powder River — 284 Mboe/d]
    OG --> GoA[Gulf of America<br/>132 Mboe/d, 96 gross wells]
    OG --> Algeria[Algeria<br/>28 Mboe/d, 219 gross wells]
    OG --> Oman[Oman<br/>72 Mboe/d — Blocks 9, 27, 53, 62, 65]
    OG --> UAE[UAE — Al Hosn Gas<br/>89 Mboe/d]
    OG --> Dolphin[Qatar/UAE — Dolphin<br/>40 Mboe/d, 24.5% interest]
    MID --> WES[Western Midstream<br/>~46% equity interest, MLP]
    MID --> Mkt[Crude & Gas Marketing<br/>oil/gas/power trading book]
    MID --> OLCV[Oxy Low Carbon Ventures]
    OLCV --> Stratos[STRATOS DAC<br/>up to 500 ktpa CO2]
    OLCV --> Seq[Sequestration Hubs<br/>Permian + Gulf Coast]
    OLCV --> Li[Lithium Development<br/>partnership with BHE Renewables]
    OLCV --> CE[Carbon Engineering<br/>acquired 2023 for $1.1B]
```
...
```mermaid
pie title FY2025 net sales by geography (continuing operations, USD billions)
    "United States" : 17.4
    "Algeria" : 0.7
    "Oman" : 1.2
    "UAE" : 1.5
...
```mermaid
quadrantChart
    title US Oil & Gas Producers — Scale vs. Diversification (2025-26)
    x-axis "Pure Permian / Independent" --> "Integrated / Diversified"
    y-axis "Smaller" --> "Larger"
    quadrant-1 "Integrated majors"
    quadrant-2 "Mid-size diversified"
    quadrant-3 "Mid-size independents"
    quadrant-4 "Pure-play independents"
    "ExxonMobil": [0.95, 0.95]
    "Chevron":    [0.85, 0.88]
    "ConocoPhillips":[0.65, 0.78]
    "Occidental": [0.45, 0.62]
    "EOG":        [0.30, 0.55]
    "Diamondback":[0.18, 0.50]
    "APA":        [0.40, 0.32]
    "Devon":      [0.35, 0.42]
    "Permian Resources":[0.12, 0.38]
    "Civitas":    [0.10, 0.28]
```
...
**Chevron (CVX, ~$305 B).** Post-Hess close (2025), Chevron's Guyana Stabroek interest and Gulf of America deepwater portfolio are now closer competitive surfaces. Permian production ~1.0 mb/d. Advantages: capital discipline, balance sheet, dividend track record. Vulnerabilities: less Permian inventory depth than OXY post-CrownRock; carbon strategy is less differentiated than OLCV ([Chevron 2024 10-K filing index — SEC EDGAR](https://www.sec.gov/cgi-bin/browse-edgar?action=getcompany&CIK=0000093410&type=10-K)).

**EOG Resources (EOG, ~$66 B).** The reference low-cost U.S. unconventional operator with Eagle Ford / Permian Delaware / Powder River exposure. Highest U.S. unconventional returns on capital across the cycle. Advantages: capital efficiency, conservative balance sheet. Vulnerabilities: no international upstream, no carbon-management franchise. Trades at the most demanding P/E in the peer set (~13× TTM) on this quality bar ([EOG Resources statistics — Stockanalysis.com](https://stockanalysis.com/stocks/eog/statistics/)).

**Diamondback Energy (FANG, ~$50 B).** Post-Endeavor, FANG is the largest pure-play Permian operator at ~600 Mboe/d ([Diamondback-Endeavor merger announcement, 2024-02-12](https://www.diamondbackenergy.com/news-releases/news-release-details/diamondback-energy-inc-and-endeavor-energy-resources-lp-merge)). Advantages: lowest pure-Permian cost structure, simplicity. Vulnerabilities: undiversified — purely a Permian commodity-price call.

**ConocoPhillips (COP, ~$130 B).** Post-Marathon Oil acquisition (2024), COP is a diversified independent with Permian, Eagle Ford, Bakken and Alaska / international LNG exposure. Most similar to OXY in shape but without the carbon-management overlay ([ConocoPhillips 10-K filings — SEC EDGAR](https://www.sec.gov/cgi-bin/browse-edgar?action=getcompany&CIK=0001163165&type=10-K)).

**APA / Apache (~$10 B).** Egypt, North Sea, Permian, and Suriname exploration. Smaller, more concentrated risk ([APA Corporation 10-K filings — SEC EDGAR](https://www.sec.gov/cgi-bin/browse-edgar?action=getcompany&CIK=0001841666&type=10-K)).

**Devon Energy (~$30 B).** Permian, Anadarko, Eagle Ford, Powder River, Williston. A direct Permian + multi-basin competitor ([Devon Energy 10-K filings — SEC EDGAR](https://www.sec.gov/cgi-bin/browse-edgar?action=getcompany&CIK=0001090012&type=10-K)).

**Permian Resources (~$15 B), Civitas (~$10 B), SM Energy, Matador, Ovintiv.** The next ring of U.S. independents; all sub-scale to OXY post-CrownRock ([2025 Form 10-K, Competition section](https://www.sec.gov/Archives/edgar/data/797468/000162828026009059/oxy-20251231.htm)).

**International concession overlap.** In Oman, OXY competes with BP (Block 61 gas), Shell, TotalEnergies and PDO consortium; OXY's incumbent Mukhaizna EOR position has very high switching costs ([Oxy Oman Block 53 Extension, 2025-05-18](https://www.oxy.com/news/news-releases/oxy-oman-and-oman-government-sign-agreement-to-extend-block-53-operations/)). In Algeria, Sonatrach is the host; ENI, BP, Repsol and Equinor are co-licensees. In the UAE, OXY's Al Hosn role is bespoke and effectively un-substitutable ([2025 Form 10-K, International operations](https://www.sec.gov/Archives/edgar/data/797468/000162828026009059/oxy-20251231.htm)).

**Carbon-management ring.** Climeworks (Switzerland — Mammoth plant operating since 2024 at 36 ktpa nameplate ([Climeworks switches on Mammoth, 2024-05-08](https://climeworks.com/press-release/climeworks-switches-on-worlds-largest-direct-air-capture-plant-mammoth))), Heirloom (mineralisation-based), CarbonCapture Inc., and Stripe Frontier-backed companies (Charm, Vesta). Industry-wide, **STRATOS at 500 ktpa is the largest operating DAC facility planned through 2026** — a meaningful first-mover scale lead ([RBN Energy — STRATOS DAC Q2-2026 startup outlook](https://rbnenergy.com/daily-posts/analyst-insight/stratos-dac-project-target-q2-2026-startup)). Competitors compete more on technology / efficiency than on volume.

**Competitive advantages — synthesised** ([2025 Form 10-K, Strategy / Competitive Strengths](https://www.sec.gov/Archives/edgar/data/797468/000162828026009059/oxy-20251231.htm)):
...
- Berkshire validation — the largest single shareholder is a known long-cycle capital ally ([Berkshire 32.7% Economic Stake in OXY — Schedule 13G/A, 2025-08-14](https://www.stocktitan.net/sec-filings/OXY/schedule-13g-a-occidental-petroleum-corporation-sec-filing-61832410029f.html))
- Investment-grade ratings (Baa3 / BBB-) restored / maintained through two transformative M&A cycles

**Competitive vulnerabilities** ([2025 Form 10-K, Risk Factors](https://www.sec.gov/Archives/edgar/data/797468/000162828026009059/oxy-20251231.htm)):
...
**5. Environmental and remediation liability tail (medium).** Even after the OxyChem sale, OXY retained legacy environmental liabilities through Environmental Resource Holdings, LLC, and entered into a guaranty in favour of Berkshire on pre-closing OxyChem liabilities ([OxyChem sale 8-K, 2026-01-02](https://www.sec.gov/Archives/edgar/data/0000797468/000162828026000153/exhibit991pressrelease1-2x.htm)). Tronox / Anadarko-era legacy issues remain in workout. Mitigant: reserves are recurring and disclosed; remediation activity is managed by Glenn Springs Holdings ([2025 Form 10-K, Item 3 Legal Proceedings](https://www.sec.gov/Archives/edgar/data/797468/000162828026009059/oxy-20251231.htm)).

### Industry / market risks

**6. Commodity price risk (high).** OXY remains a price-taker on roughly 1.4 Mboe/d of production. The 2025 average WTI of $64.81 was already down 14% YoY ([2025 Form 10-K, Commodity Pricing](https://www.sec.gov/Archives/edgar/data/797468/000162828026009059/oxy-20251231.htm)); a sustained sub-$60 WTI environment would compress 2026 free cash flow by ~$2 B vs. budget on our math. Mitigant: 2026 crude two-way collars added in February 2026 ([Q1-2026 10-Q, Hedges Note](https://www.sec.gov/Archives/edgar/data/0000797468/000162828026030584/oxy-20260331.htm)); structurally lower lease operating cost per Boe ($8.94 in 2025 vs. $10.48 in 2023); long-life international barrels are less sensitive to short-term price.

**7. Competitive intensity in the Permian (medium-high).** Post-XOM/Pioneer and FANG/Endeavor, the basin is dominated by two-three operators with multi-decade inventories ([ExxonMobil-Pioneer merger announcement, 2023-10-11](https://corporate.exxonmobil.com/news/news-releases/2023/1011_exxonmobil-announces-merger-with-pioneer-natural-resources-in-an-all-stock-transaction); [Diamondback-Endeavor merger, 2024-02-12](https://www.diamondbackenergy.com/news-releases/news-release-details/diamondback-energy-inc-and-endeavor-energy-resources-lp-merge)). OXY's CrownRock acquisition kept it in the top tier, but per-foot capital efficiency continues to favour FANG / EOG. Mitigant: CO₂-EOR optionality + scale + integration with OLCV.

**8. Regulatory / IRA policy risk (medium).** The 45Q tax credit and other IRA / OBBBA provisions underwrite OLCV economics. A material reduction of 45Q would materially impair STRATOS NPV. Mitigant: 45Q is well-entrenched bipartisan policy, but headline risk remains ([2025 Form 10-K, Regulatory Matters / 45Q](https://www.sec.gov/Archives/edgar/data/797468/000162828026009059/oxy-20251231.htm)).

**9. Energy transition / demand-destruction risk (medium-long term).** EV penetration, fuel-efficiency improvements and policy actions could reduce global oil demand on a 10–20 year horizon ([IEA — World Energy Investment 2025](https://www.iea.org/reports/world-energy-investment-2025)). Mitigant: OXY's portfolio — Permian + international long-life PSAs + carbon-management hedge — is among the most transition-resilient in U.S. independents ([2025 Form 10-K, Sustainability / Net-Zero strategy](https://www.sec.gov/Archives/edgar/data/797468/000162828026009059/oxy-20251231.htm)).
...
**11. Preferred stock overhang and dividend constraint ($8.5 B face, 8% coupon → 9% on default — high).** Berkshire's $8.5 B face preferred currently generates $679 M / year of dividend obligations, ranks ahead of common in liquidation, and contains a mandatory-redemption-with-premium feature that requires OXY to redeem $1.10 of preferred for every $1 of common returns above $4.00 / share / year ([Berkshire Securities Purchase Agreement 8-K Ex 10-1, 2019](https://www.sec.gov/Archives/edgar/data/0000797468/000095015719000529/ex10-1.htm)). Mitigant: OXY can voluntarily redeem after August 2029 at a 5% premium; the structure incentivises measured common returns. The instrument is dilutive to common — every $1 paid to preferred or in preferred redemption premium is $1 not available to common ([2025 Form 10-K, Preferred Stock Note](https://www.sec.gov/Archives/edgar/data/797468/000162828026009059/oxy-20251231.htm)).
...
![Principal debt trend](charts/oxy_debt_trend.png)
...
![Production by region](charts/oxy_production_region.png)
...
![Segment earnings](charts/oxy_segment_earnings.png)
...
![Peer valuation](charts/oxy_peer_valuation.png)
