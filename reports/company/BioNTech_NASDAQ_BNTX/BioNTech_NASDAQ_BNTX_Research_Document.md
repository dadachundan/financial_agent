...
```mermaid
xychart-beta
    title "BioNTech revenue vs R&D expense (€m, FY23–FY25)"
    x-axis ["FY23", "FY24", "FY25"]
    y-axis "EUR (millions)" 0 --> 5000
    bar [3819.0, 2751.1, 2869.9]
    line [1783.1, 2254.2, 2104.9]
```
...
```mermaid
timeline
    title BioNTech SE — major milestones
    2008 : Founded in Mainz by Sahin and Türeci
    2018 : Pfizer flu collaboration (foundation for 2020 COVID pivot)
    2019 : NASDAQ IPO at USD 15/ADS — valuation USD 3.4 bn
    2020 : BNT162b2 EUA — first mRNA medicine ever
    2021 : Comirnaty BLA, peak revenue €18.9 bn
    2023 : InstaDeep acquired for AI-native discovery
    2024 : Biotheus acquired — pumitamig ex-China rights
    2025 : BMS deal on pumitamig — USD 3.5 bn non-contingent
    2025 : CureVac merger closes Dec — €1.3 bn all-share
    2026 : FY26 guide €1.7–2.2 bn; oncology Phase 3 readouts
```
...
![Pipeline by stage](charts/bntx_pipeline_stage.png)
...
```mermaid
graph TD
    A[BioNTech SE] --> B[A. Commercial COVID franchise]
    A --> C[B. Oncology — pumitamig anchor]
    A --> D[C. ADC portfolio]
    A --> E[D. iNeST + FixVac mRNA cancer]
    A --> F[E. Infectious-disease vaccines]
    A --> G[F. InstaDeep AI platform]
    B --> B1[Comirnaty BNT162b2 — w/ Pfizer]
    B --> B2[Variant-updated strains seasonal]
    C --> C1[Pumitamig BNT327 1L NSCLC Ph3]
    C --> C2[Pumitamig 1L ES-SCLC Ph3]
    C --> C3[Pumitamig 1L HCC / TNBC / CRC Ph2]
    D --> D1[BNT323 HER2-ADC Ph3 EC]
    D --> D2[BNT324 B7H3-ADC Ph1/2]
    D --> D3[BNT325 TROP2-ADC Ph1/2]
    D --> D4[BNT326 HER3-ADC Ph1/2]
    E --> E1[Autogene cevumeran iNeST PDAC Ph2/3]
    E --> E2[BNT111 melanoma Ph2]
    E --> E3[BNT113 HNSCC Ph2 — FDA Fast Track]
    E --> E4[BNT211 CLDN6 CAR-T Ph1/2]
    F --> F1[BNT163 HSV-2 Ph1]
    F --> F2[BNT165 malaria Ph1]
    F --> F3[BNT166 mpox]
    F --> F4[BNT164 TB Ph1]
```
...
```mermaid
pie title FY25 revenue by customer (consolidated, €m)
    "Pfizer (56%)" : 1602.0
    "German FedMin of Health (22%)" : 627.5
    "Bristol-Myers Squibb (21%)" : 613.0
    "Other (1%)" : 27.4
```
...
![Customer concentration legacy chart](charts/bntx_customer_concentration.png)
...
![Geographic mix legacy chart](charts/bntx_geographic_mix.png)
...
```mermaid
xychart-beta
    title "Pumitamig peak-sales TAM by indication (USD bn, peak; 100% basis)"
    x-axis ["NSCLC", "SCLC", "HCC", "TNBC", "HNSCC", "CRC"]
    y-axis "USD bn (peak; 100% basis)" 0 --> 25
    bar [20, 3, 3, 3, 2, 4]
```
...
![BNTX peer valuation legacy chart](charts/bntx_peer_valuation.png)
...
quadrantChart
    ...
    quadrant-4 Subscale, broad pipeline
    BioNTech: [0.80, 0.95]
    Moderna: [0.60, 0.55]
    Summit Therapeutics: [0.30, 0.55]
    Akeso: [0.55, 0.40]
    Iovance: [0.25, 0.20]
    Arcus: [0.45, 0.15]
    Daiichi Sankyo: [0.85, 0.85]
```
...
```mermaid
xychart-beta
    title "From today's EV to risk-adjusted 2032 peak revenue (USD bn)"
    x-axis ["Today EV", "Pumitamig NSCLC", "Pumitamig SCLC/HCC/TNBC", "ADCs", "BNT122 iNeST", "BNT113 FixVac", "COVID franchise", "2032 enterprise value"]
    y-axis "USD bn" 0 --> 40
    bar [9, 12, 6, 3, 4, 1, 5, 40]
```
...
![Revenue + R&D legacy chart](charts/bntx_revenue_rd_trajectory.png)
![Cash runway legacy chart](charts/bntx_cash_runway.png)
