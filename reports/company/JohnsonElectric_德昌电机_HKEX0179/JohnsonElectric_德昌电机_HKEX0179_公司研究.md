...
```mermaid
timeline
    title 德昌电机集团 1959–2025 关键节点
    1959 : 香港家族创业 — 汪松亮 / 汪顾亦珍创办
    1970s : 设立美国销售处 + 进入汽车零部件
    1984 : 在香港联交所主板上市 (股份代号 0179)
    1989 : 中国深圳沙井设厂 — 第一波内地扩张
    1995–2005 : 收购 UTA 的 Gate 业务 + 进入印度 / 塞尔维亚区域枢纽
    2008 : 收购加拿大 Stackpole International (粉末冶金 / powder metal)
    2014 : 收购 AML
    2016 : 收购瑞士 SAIA-Burgess Electronics 与美国 Parlex Corp.
    2020 : 收购德国 E. Zimmermann (汽车精密部件)
    2022 : 收购 Pendix GmbH 80% 股权 (e-bike 中置电机)
    2025-07 : 与上海机电合资组建动界智控 (Jointelligence) 切入人形机器人关节模组
```
...
```mermaid
graph TD
    JE[Johnson Electric / 德昌电机集团] --> APG[Automotive Products Group 汽车产品组别<br/>FY25/26: USD 3,054m / 84% 收入]
    JE --> IPG[Industry Products Group 工商用产品组别<br/>FY25/26: USD 596m / 16% 收入]
    JE --> CnS[Components & Services 组件与服务]

    APG --> APG1[制动 Braking — Brake booster / EPB / EVP]
    APG --> APG2[闭合 Closure — Window lift / Power liftgate / Sunroof]
    APG --> APG3[内饰 Interior — Seat motor / Steering column adjuster]
    APG --> APG4[泵类 Pumps — E-water pump / E-oil pump]
    APG --> APG5[转向 Steering — EPS / Steering-by-wire]
    APG --> APG6[散热 Thermal Management — ITMS / Cooling fan module]
    APG --> APG7[粉末金属 Powder Metal Components]
    APG --> APG8[视觉 Vision — Headlamp actuator / LiDAR motor]

    IPG --> IPG1[商用设备 Business equipment]
    IPG --> IPG2[数据中心冷却 Data center cooling — DC pumps]
    IPG --> IPG3[电单车 E-bikes — Pendix 中置驱动]
    IPG --> IPG4[家庭自动化 Home automation]
    IPG --> IPG5[园艺工具 Lawn & garden]
    IPG --> IPG6[医疗设备 Medical devices]
    IPG --> IPG7[电动工具 Power tools]
    IPG --> IPG8[机器人 Robotics — 人形机器人关节模组]
    IPG --> IPG9[智能计量 Smart metering]
    IPG --> IPG10[白色家电 White goods]

    CnS --> CnS1[轴承 Bearings]
    CnS --> CnS2[换向器 Commutators]
    CnS --> CnS3[压铸件 Die cast components]
    CnS --> CnS4[磁体 / 层压件 Magnets & Laminations]
    CnS --> CnS5[PCBA 印刷电路板组件]
```
...
```mermaid
graph LR
    Core[核心能力 / Core Capabilities<br/>1. 微型 / 中型 DC 与 BLDC 电机设计<br/>2. 粉末冶金 / 精密注塑<br/>3. 全球 30,000+ 人制造网络<br/>4. ISO/TS 16949 汽车级质量体系] --> APG_App[APG 应用 — 现金奶牛]
    Core --> IPG_New[IPG 新赛道 — 第二曲线]

    APG_App --> Therm[散热管理 — NEV CPV 提升]
    APG_App --> Brake[制动 / 转向 — 线控底盘]
    APG_App --> Vision[视觉 / 内饰 / 闭合]

    IPG_New --> DC_AI[数据中心液冷 — AI 基建]
    IPG_New --> Robot[人形机器人关节模组 — 动界智控 JV]
    IPG_New --> EBike[e-bike 中置驱动 — Pendix]
    IPG_New --> Medical[医疗 / 半导体精密]

    DC_AI -.短期收入兑现.-> ShortTerm[FY25/26 已贡献增长]
    Robot -.远期期权.-> LongTerm[FY26/27+ 待具名客户]
```
...
```mermaid
pie title FY25/26 集团营收按组别分布 (USD million)
    "APG / 汽车产品组别 3,054m (84%)" : 3054
    "IPG / 工商用产品组别 596m (16%)" : 596
```

```mermaid
pie title FY25/26 APG 营收按地区分布 (近似口径, 基于上半年比例外推)
    "亚太 Asia-Pacific 约 34%" : 34
    "EMEA 欧洲 / 中东 / 非洲 约 30%" : 30
    "美洲 Americas 约 36%" : 36
```
...
```mermaid
quadrantChart
    title 0179.HK 在不同赛道的竞争位置 (示意性, 非定量)
    x-axis "技术壁垒低" --> "技术壁垒高"
    y-axis "市场地位弱" --> "市场地位强"
    quadrant-1 "高壁垒 / 强地位"
    quadrant-2 "低壁垒 / 强地位"
    ...
    quadrant-4 "高壁垒 / 弱地位"
    "汽车散热泵 ITMS": [0.7, 0.8]
    "汽车制动执行器 EVP/EPB": [0.7, 0.75]
    "汽车视觉 / 闭合电机": [0.4, 0.7]
    "数据中心液冷泵 (CCM)": [0.65, 0.6]
    "人形机器人关节模组": [0.85, 0.35]
    "e-bike 中置驱动 (Pendix)": [0.6, 0.4]
    "粉末冶金件 Stackpole": [0.5, 0.75]
    "白色家电 / 园艺电机": [0.25, 0.7]
```
