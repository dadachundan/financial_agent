...
```mermaid
xychart-beta
    title "ISRG 营收与 GAAP 毛利率（2023–2025A，单位：$bn / %）"
    x-axis [2023, 2024, 2025]
    y-axis "营收 $bn / 毛利率 %" 0 --> 80
    bar [71.2, 83.5, 100.6]
    line [66.4, 67.5, 66.0]
```
...
*结论：ISRG 过去一年既跑输大盘（−41.6 pts）也略跑输器械板块（−2.6 pts），是一次"板块 + 个股双重 de-rate"——而基本面（procedure +18%、utilization 重新加速）没有同步恶化，这正是估值与质量背离的核心读点。来源：[yfinance ISRG/^GSPC/IHI 收盘价](https://finance.yahoo.com/quote/ISRG/)；参考点见 [Bernstein — ISRG 4Q25, 2026-01-23, p.1](http://xs-macbook-air.local:5001/zsxq/pdf/585525425551554/Bernstein-Intuitive%20Surgical%20Inc%EF%BC%88ISRG.US%EF%BC%89Intuitive%20Surgical%204Q25%EF%BC%9ASqueaky%20clean%20quarter%EF%BC%9Btop%20pick%EF%BC%88PT%20%24750%EF%BC%89-260123.pdf)（其截至 2026-01-23 的 12M 绝对 −13.9% vs SPX +13.6% → 相对 −27.5%，本表已刷新至最新）。*
...
```mermaid
timeline
    title Intuitive Surgical 关键里程碑
    1995 : 公司在加州 Sunnyvale 注册成立（Moll/Younge/Freund）
    2000 : 首代 da Vinci 系统获 FDA 批准；NASDAQ IPO
    2003 : 与竞争对手 Computer Motion 合并，消除主要直接竞争
    2014 : da Vinci Xi 推出（第四代，模块化臂架）
    2018 : da Vinci SP（单孔单臂）上市；Ion 平台研发推进
    2019 : Ion 腔内系统获 FDA 批准（肺活检）；中国复星合资落地
    2024 : da Vinci 5（第五代，力反馈 + 10000× 算力）上市
    2025 : Dave Rosa 接任 CEO；推出 XiR refurbished；procedure +18%
    2026 : da Vinci 5 获心脏外科 FDA 清单（~16 万台/年机会）
```
...
```mermaid
graph TD
    A["da Vinci / Ion 系统<br/>(razor 剃刀，资本设备)"] --> B["外科医生训练 + 认证<br/>(SimNow / 转换成本)"]
    B --> C["每台手术消耗 I&A 耗材<br/>(blade 刀片，限次使用)"]
    C --> D["Service 服务合同<br/>(安装/维护/再处理)"]
    C --> E["utilization↑<br/>(单机手术量)"]
    E --> F["recurring 年金放大<br/>(86% 经常性收入)"]
    F --> G["现金流再投入 R&D<br/>(da Vinci 5 / cardiac / AI)"]
    G --> A
    D --> F
```
...
```mermaid
pie title 2025 营收结构（按收入流，占总营收 %）
    "Instruments & accessories（耗材）" : 60
    "Systems（系统）" : 25
    "Service（服务）" : 16
```
...
```mermaid
quadrantChart
    title 手术机器人竞争定位（装机量规模 vs 软组织聚焦度）
    x-axis "低装机量规模" --> "高装机量规模"
    y-axis "多模态/分散" --> "软组织机器人聚焦"
    quadrant-1 "聚焦 + 规模（领导）"
    quadrant-2 "聚焦 + 早期"
    quadrant-3 "分散 + 早期"
    quadrant-4 "分散 + 规模"
    "ISRG da Vinci": [0.93, 0.9]
    "Medtronic Hugo": [0.4, 0.78]
    "J&J Ottava": [0.25, 0.7]
    "MicroPort MedBot": [0.32, 0.6]
    "Stryker Mako (骨科)": [0.72, 0.2]
```
...
```mermaid
xychart-beta
    title "全球手术机器人 TAM（$bn，三方 + GS 测算）"
    x-axis [2025, 2030, 2035]
    y-axis "TAM $bn" 0 --> 50
    bar [11.5, 25, 42]
    line [9, 11, 12]
```
...
9. **估值/倍数压缩风险（高）。** *分析师观点：* TTM P/E 51.5× >2× 行业中位，虽已较自身历史 de-rate，但若 procedure 增速减速或板块资金流出，仍可能继续 de-rate。触发因素：增长减速、利率上行、板块轮动、对 guidance 的 miss。当前 forward P/E ~36× vs 5 年均值 ~56×，下行空间已收窄但非为零（[Yahoo Finance — ISRG](https://finance.yahoo.com/quote/ISRG/key-statistics/)；[UBS — ISRG, p.1](http://xs-macbook-air.local:5001/zsxq/pdf/585581521118154/UBS-Intuitive%20Surgical%20Inc%EF%BC%88ISRG.US%EF%BC%89Solid%20Q1%EF%BC%8C%20Utilization%20Flywheel%20%2B%20System%20Placements%20Likely%20to%20Sustain%20Momentum-260423.pdf)）。
...
**Debate 3 —— "估值太贵，TTM P/E 51× 没有安全边际。"** *分析师观点：* 关键在于看 forward 而非 TTM——Bernstein 的封面矩阵显示 Adj P/E 随盈利成长由 58.9× 压到 44.9×（F25A→F27E），当前 forward P/E ~36× 已低于 ISRG 自身 ~56× 的 5 年均值；UBS 虽因估值维持 Neutral，但其框架也建立在 ~48× forward P/E 之上，并非看空基本面（[Bernstein — ISRG 4Q25, p.1](http://xs-macbook-air.local:5001/zsxq/pdf/585525425551554/Bernstein-Intuitive%20Surgical%20Inc%EF%BC%88ISRG.US%EF%BC%89Intuitive%20Surgical%204Q25%EF%BC%9ASqueaky%20clean%20quarter%EF%BC%9Btop%20pick%EF%BC%88PT%20%24750%EF%BC%89-260123.pdf)；[UBS — ISRG, p.1](http://xs-macbook-air.local:5001/zsxq/pdf/585581521118154/UBS-Intuitive%20Surgical%20Inc%EF%BC%88ISRG.US%EF%BC%89Solid%20Q1%EF%BC%8C%20Utilization%20Flywheel%20%2B%20System%20Placements%20Likely%20to%20Sustain%20Momentum-260423.pdf)）。
...
- zsxq 引用 5 个 file_id 经 `find_pdf.py` 确认存在且 `local_exists=true`：585525425551554（Bernstein）、585581521118154（UBS）、184482814241482（BofA）、585582881584284（GS）、212485545248851（Citi）；路由用 `/zsxq/pdf/<id>`（非死链 `/zsxq-pdf/`）。
