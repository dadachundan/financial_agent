...
```mermaid
timeline
    title Analog Devices — Selected Milestones
    1965 : Founded in Cambridge, MA by Ray Stata and Matthew Lorber
    1969 : Initial public offering on NYSE
    1977 : Introduced industry-first monolithic A/D converter (AD571)
    1987 : Acquired Precision Monolithics, expanding amplifier portfolio
    1996 : Vincent Roche joins (already an ADI exec since 1988) - launched MEMS / iCoupler franchise late 1990s
    2007 : Listed on Nasdaq (transferred from NYSE)
    2013 : Vincent Roche named President & CEO
    2014 : Acquired Hittite Microwave for $2.5 B - RF/microwave foundation
    2017 : Acquired Linear Technology for $14.8 B - became #2 analog supplier
    2021 : Closed Maxim acquisition (~$28 B in stock) - became >$10 B revenue franchise
    2022 : Vincent Roche named Chair of the Board
    2024 : Richard Puccio joins as CFO from AWS - February
    2025 : Returned >$4 B to shareholders; FY2025 revenue $11.02 B as cyclical recovery began
    2026 : Q2 FY26 revenue $3.62 B, up 37% YoY - all three B2B markets at record bookings
```
...
graph TD
    ADI[Analog Devices]
    ADI --> AMS[Analog & Mixed-Signal]
    ADI --> PMR[Power Mgmt & Reference]
    ADI --> ARF[Amplifiers / RF & Microwave]
    ADI --> SA[Sensors & Actuators]
    ADI --> DSP[DSP & System Products]
    ADI --> SW[Software / AI / Digital Platforms]
    AMS --> ADC[ADCs / DACs - data converters]
    AMS --> Precision[Precision converters]
    AMS --> HS[High-speed converters]
    PMR --> SwReg[Switching regulators]
    PMR --> LDO[Linear regulators]
    PMR --> BMS[Battery management - automotive / industrial]
    PMR --> uModule[uModule integrated power - ex-Linear]
    ARF --> OpAmp[Op-amps / instrumentation]
    ARF --> RFMW[RF / microwave ICs]
    ARF --> Beamform[Beamforming for 5G / satellite]
    SA --> MEMS[MEMS IMU / accelerometer / gyroscope]
    SA --> Iso[iCoupler digital isolators]
    SA --> TempMag[Temperature / magnetic sensors]
    DSP --> Sharc[SHARC family]
    DSP --> Blackfin[Blackfin family]
    SW --> CFS[CodeFusion Studio 2.0]
    SW --> PowerStudio[Power Studio]
```
...
```mermaid
pie title FY2025 Revenue by End Market
    "Industrial" : 45
    "Automotive" : 30
    "Consumer" : 13
    "Communications" : 13
...
```mermaid
quadrantChart
    title Analog Semiconductor Competitive Positioning
    x-axis "Lower Performance / Cost-led" --> "Highest Performance / Premium"
    y-axis "Narrow Portfolio" --> "Broad Portfolio"
    quadrant-1 "Premium Broad-Line Leaders"
    quadrant-2 "Niche Premium Specialists"
    quadrant-3 "Long-tail Commodity"
    quadrant-4 "Mass-market Broad-Line"
    "ADI": [0.85, 0.78]
    "TXN": [0.65, 0.92]
    "Infineon": [0.55, 0.78]
    "STMicro": [0.50, 0.70]
    "NXPI": [0.55, 0.55]
    "Microchip": [0.40, 0.62]
    "Skyworks/Qorvo": [0.75, 0.25]
    "SG Micro / 3Peak": [0.30, 0.30]
```
