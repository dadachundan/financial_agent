...
```mermaid
timeline
    title Teradyne — selected milestones, 1960–2026
    1960 : Founded in Boston by Alex d'Arbeloff and Nick DeWolf
    1970 : IPO on NYSE
    2008 : Universal Robots (then independent) launches UR5 — the world's first commercially viable cobot
    2011 : Acquires LitePoint (wireless test) for ~$580m
    2015 : Acquires Universal Robots (cobot arms, Denmark) for $285m
    2018 : Acquires Mobile Industrial Robots (AMRs, Denmark) and Energid (control software)
    2019 : Acquires Lemsys and AutoGuide (tuck-ins for power test and AMR fleet)
    2020 : Acquires DIS (device interface boards) — later divested in 2024
    2023 : Founder-era CEO Mark Jagiela retires; Greg Smith becomes CEO (Jan 2023). Board authorises $2.0bn buyback
    2024 : Strategic partnership with Technoprobe — Teradyne acquires 10% of TPRO equity for $440m; sells DIS business for $90.3m gain
    2025 : Acquires Infineon's automated test equipment ("AET") technology / team for €17.6m (Jan); acquires Quantifi Photonics (PIC test) for $127.2m (May); restructures Robotics (-400 heads); creates "Product Test" segment in March
    2026 : Announces MultiLane Test Products JV (75% TER, $157m) for AI-data-center I/O test (Jan); Michelle Turner becomes CFO; record Q1 with 70% AI-driven revenue
```
...
```mermaid
graph TD
    A[Teradyne] --> B[Semiconductor Test 79% of FY25 rev]
    A --> C[Product Test 11%]
    A --> D[Robotics 10%]

    B --> B1[FLEX Test Platform - SoC]
    B1 --> B1a[UltraFLEXplus PACE arch.]
    B1 --> B1b[UltraFLEX]
    B1 --> B1c[FLEX]
    B --> B2[J750 - microcontroller / image sensor]
    B2 --> B2a[IP750 Image Sensor]
    B --> B3[Magnum - memory test]
    B3 --> B3a[Magnum 7 - flash/DRAM/HBM/MCP]
    B3 --> B3b[Magnum EPIC - final test]
    B --> B4[ETS / Eagle - analog/power]
    B4 --> B4a[ETS-88 SiC/GaN]
    B4 --> B4b[ETS-800]
    B --> B5[Integrated System Test IST]
    B5 --> B5a[SLT - system-level test]
    B5 --> B5b[HDD/SSD testers]
    B --> B6[AET ex-Infineon, Jan-25]
    B --> B7[MLTP JV - AI I/O test, 2026]

    C --> C1[LitePoint Wireless Test]
    C --> C2[Circuit-board test]
    C --> C3[Defense & Aerospace test]
    C --> C4[Quantifi Photonics PIC test, May-25]

    D --> D1[Universal Robots cobot arms]
    D1 --> D1a[UR3e, UR7e, UR8 Long, UR12e, UR15, UR16e, UR18, UR20, UR30]
    D1 --> D1b[PolyScope SW / UR+ ecosystem]
    D --> D2[Mobile Industrial Robots AMRs]
    D2 --> D2a[MiR250, MiR600, MiR1350]
    D2 --> D2b[MiR1200 Pallet Jack]
```
...
```mermaid
pie title FY2025 revenue — direct-customer concentration
    "Top-1 direct customer (~19%)" : 19
    "Two 10%+ specifying customers (~22%)" : 22
    "Customers 4-5 (~3%)" : 3
    "All other (~56%)" : 56
```
...
```mermaid
quadrantChart
    title Semi Test competitive position — AI/HBM exposure vs platform breadth
    x-axis "Low AI/HBM exposure" --> "High AI/HBM exposure"
    y-axis "Narrow platform" --> "Broad platform"
    quadrant-1 "Broad + High AI"
    quadrant-2 "Broad + Low AI"
    quadrant-3 "Narrow + Low AI"
    quadrant-4 "Narrow + High AI"
    Advantest: [0.88, 0.85]
    Teradyne:  [0.82, 0.90]
    Cohu:      [0.30, 0.55]
    SPEA:      [0.25, 0.30]
