...
```mermaid
timeline
    title Credo Technology — Selected Milestones
    2008 : Credo founded in San Jose by ex-Marvell engineers
    2013 : Bill Brennan named CEO of predecessor entity
    2020 : HiWire AEC product line launched
    2022 : IPO on Nasdaq (January 27, 2022) — IPO price $10.00
    2023 : Microsoft AEC dual-ToR architecture publicly disclosed
    2024 : Astera Labs IPO frames "AI interconnect" sector
    2025 : Q4 FY25 revenue $170M, +179% YoY; FY25 first GAAP-profitable year
    2025 : ATM offering raises $736M; Hyperlume acquired for $92M
    2026 : Q3 FY26 revenue $407M, run-rate >$1.6B
```
...
```mermaid
graph TD
    Credo[Credo Technology] --> IC[Integrated Circuits]
    Credo --> AEC[Active Electrical Cables]
    Credo --> CHIP[SerDes Chiplets]
    Credo --> IP[SerDes IP licensing]
    IC --> ODSP[Optical PAM4 DSPs]
    IC --> PCIE[PCIe Retimers]
    IC --> LCPHY[Line Card PHYs]
    IC --> ALC[Active Linear Cable controllers]
    IC --> GBOX[OmniConnect Gearbox / memory]
    AEC --> HW[HiWire AECs]
    AEC --> ZFAEC[ZeroFlap AECs]
    AEC --> ZFOPT[ZeroFlap Optical Transceivers]
    CHIP --> MCM[Multi-Chip Module SerDes Chiplets]
```
...
```mermaid
pie title End-customer revenue mix — Q3 FY26 (Three months ended Jan 31, 2026)
    "Customer B (39%)" : 39
    "Customer D (32%)" : 32
    "Customer E (17%)" : 17
    "All other (12%)" : 12
