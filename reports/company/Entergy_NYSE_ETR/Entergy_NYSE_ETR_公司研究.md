...
```mermaid
xychart-beta
    title "Entergy 合并营收 vs 归母净利润（USD bn）"
    x-axis [2023, 2024, 2025]
    y-axis "USD bn" 0 --> 15
    bar [12.15, 11.88, 12.95]
    line [2.36, 1.06, 1.76]
```
...
timeline
    title Entergy 关键里程碑
    1949 : Middle South Utilities 成立
    1989 : 更名为 Entergy Corporation；总部迁至新奥尔良
    1993 : 并购 Gulf States Utilities（得克萨斯东部 + 路易斯安那）
    1999-2007 : 通过 Entergy Wholesale Commodities 扩张非监管核电
    2017-2022 : 战略性退出非监管核电，处置 FitzPatrick / Pilgrim / Indian Point / Palisades
    2022 : Drew Marsh 出任 Chair & CEO，从 CFO 转任
    2024 : 公布服务 Meta Louisiana 数据中心首期 2.3 GW / USD 3.2 bn 投资
    2025 : 出售路易斯安那天然气分销业务（2025-06-30 截止）
    2026 : 与 Meta 签订 20 年扩展协议，新增 5.2 GW 燃气 + 2.5 GW 太阳能 + 储能；四年资本开支由 USD 43 bn → USD 57 bn
```
...
```mermaid
graph TD
    A[Entergy Generation Fleet] --> B[Nuclear / 核电 ~5 GW]
    A --> C[Natural Gas / 天然气 ~17-20 GW]
    A --> D[Solar / 太阳能 ~2-3 GW + 储能]
    A --> E[Coal / 煤电 衰减中]
    A --> F[Hydro / 水电 65 MW]
    B --> B1[ANO 1 & 2 / Arkansas Nuclear One]
    B --> B2[Grand Gulf / SERI 90% interest]
    B --> B3[River Bend / Entergy Louisiana]
    B --> B4[Waterford 3 / Entergy Louisiana]
    C --> C1[Combined-cycle 新建机组 - Hyperscale 配套]
    C --> C2[Existing peaker & legacy gas units]
    C1 --> C1a[Franklin Farms Units 1&2 / Meta 配套 USD 2.39 bn]
    C1 --> C1b[Waterford 5 / Meta 配套]
    C1 --> C1c[Westlake Power Station / Waterford 6]
    C1 --> C1d[Orange County Advanced / Entergy Texas]
    C1 --> C1e[Ironwood / Jefferson / Lone Star Power]
    D --> D1[Arkansas Cypress 600 MW + 350 MW 储能]
    D --> D2[Segno / Votaw / Bogalusa / Cypress Harvest Solar]
    D --> D3[Delta Solar / Penton Solar]
```
...
```mermaid
pie title FY2025 Utility 营收按持牌子公司（USD mn）
    "Entergy Louisiana 5,732" : 5732
    "Entergy Arkansas 2,784" : 2784
    "Entergy Texas 2,128" : 2128
    "Entergy Mississippi 1,956" : 1956
    "Entergy New Orleans 772" : 772
```
...
```mermaid
xychart-beta
    title "美国数据中心装机 GW（多源平均估算）"
    x-axis [2023, 2025, 2027, 2030]
    y-axis "GW" 0 --> 150
    bar [40, 70, 100, 130]
```
...
```mermaid
quadrantChart
    title 美国电力公用事业竞争格局：受监管 vs IPP × 数据中心露出
    x-axis "受监管度（低 → 高）"
    y-axis "数据中心负荷露出（低 → 高）"
    quadrant-1 "高监管 + 高 DC 露出"
    quadrant-2 "低监管 + 高 DC 露出"
    quadrant-3 "低监管 + 低 DC 露出"
    quadrant-4 "高监管 + 低 DC 露出"
    Entergy: [0.85, 0.85]
    Southern: [0.80, 0.65]
    Duke: [0.78, 0.60]
    Xcel: [0.75, 0.40]
    NextEra: [0.55, 0.55]
    Constellation: [0.25, 0.92]
    Vistra: [0.20, 0.88]
    NRG: [0.15, 0.75]
```
...
- [SWS 国防军工 商发&燃机 #2, zsxq #184152212118422](http://xs-macbook-air.local:5001/zsxq-pdf/184152212118422)
...
- **Louisiana 12.2 GW 在建燃气产能**：来自 [SWS 燃机研究 #2, zsxq #184152212118422 p.14](http://xs-macbook-air.local:5001/zsxq-pdf/184152212118422#page=14)，本地 PDF 验证。✓
