...
```mermaid
timeline
    title Advantest milestones
    1954 : Takeda Riken Industry Co. founded in Tokyo (precision instruments)
    1985 : Renamed Advantest Corporation; listed on TSE
    1991 : Launched T3300 memory tester — establishes share leadership in DRAM ATE
    2000s : Builds SoC tester franchise (T2000 series) as memory market matures
    2011 : Acquires Verigy (US$1.1bn) — gains V93000 platform; takes #1 in SoC ATE
    2015 : Acquires Essai (test sockets) for thermal control and contact tech
    2019 : Acquires Astronics Test Systems (US$100M) — enters System Level Test
    2020 : Launches V93000 EXA Scale — current flagship SoC tester
    2023 : 4-for-1 stock split (effective Oct 1, 2023)
    2024 : Doug Lefever appointed first non-Japanese Group CEO
    2024 : Third Mid-Term Plan (MTP3, FY24–FY26) launched
    2025 : Record FY2024 — ¥779.7bn sales; AI tester demand accelerates
    2026 : Record FY2025 — ¥1,128.6bn sales; FY2026 guide ¥1,420bn
```
...
```mermaid
graph TD
    A[Advantest Corporation]
    A --> B[Test System Business — ¥1,019.4bn FY2025]
    A --> C[Services & Others — ¥109.2bn FY2025]
    B --> B1[SoC Test Systems]
    B --> B2[Memory Test Systems]
    B --> B3[Mechatronics & device interfaces]
    B1 --> B1a[V93000 EXA Scale — flagship]
    B1 --> B1b[V93000 Smart Scale / Pin Scale]
    B1 --> B1c[T2000 — established platform]
    B1 --> B1d[Display driver IC / DDIC testers]
    B2 --> B2a[T5503HS2 — HBM / DDR / LPDDR]
    B2 --> B2b[T5835 — DRAM and high-speed NAND]
    B2 --> B2c[T5833 / T5833ES]
    B2 --> B2d[T5587 — non-volatile memory]
    B3 --> B3a[Test handlers - active thermal control]
    B3 --> B3b[Device interface boards - DIBs/probe cards]
    B3 --> B3c[CD-SEM nanotechnology]
    C --> C1[Support services - installed base]
    C --> C2[System-level test - Advantest Test Solutions]
    C --> C3[Test sockets / contactors - Essai]
    C --> C4[Advantest Cloud Solutions]
    C --> C5[Used equipment, consumables]
```
...
```mermaid
pie title FY2025 net sales by customer region (¥1,128.6bn total)
    "Asia ex-Japan (Taiwan, Korea, China, Malaysia, etc.)" : 1035.85
    "Americas (US, Brazil)" : 44.47
    "Europe (Germany, Israel, Ireland)" : 23.15
    "Japan" : 25.14
```
...
```mermaid
quadrantChart
    title ATE competitive positioning — share vs. portfolio breadth (CY2024)
    x-axis "Narrow portfolio" --> "Broad portfolio"
    y-axis "Low share" --> "High share"
    quadrant-1 "Share leaders, broad"
    quadrant-2 "Share leaders, narrow"
    quadrant-3 "Sub-scale, narrow"
    quadrant-4 "Broad portfolio, sub-scale"
    "Advantest": [0.92, 0.92]
    "Teradyne": [0.78, 0.65]
    "Cohu": [0.40, 0.18]
    "Chroma ATE": [0.55, 0.12]
    "Aehr": [0.18, 0.08]
    "Yoseni / CN upstarts": [0.30, 0.05]
```
