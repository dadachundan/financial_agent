...
```mermaid
timeline
    title STMicroelectronics — Key Milestones (1987–2026)
    1987 : SGS-Thomson Microelectronics formed from SGS (IT) + Thomson Semiconducteurs (FR)
    1994 : Dual IPO on NYSE and Paris Bourse (December)
    1998 : Renamed STMicroelectronics N.V. (Netherlands holding); third listing on Borsa Italiana
    2018 : Jean-Marc Chery promoted to CEO; cyclical recovery begins
    2018 : Acquires Norstel (Swedish SiC substrate maker) — first vertically-integrated SiC step
    2022 : Revenue peaks at $16.13 bn; gross margin 47.3%; SiC ramps with Tesla, Renault, BYD
    2023 : Record revenue $17.29 bn, 47.9% GM, $4.21 bn net income — peak cycle
    2023 : Sanan-ST JV signed (June 7) — 200mm SiC fab in Chongqing, China for Chinese customers
    2024 : EV slowdown + EM/Industrial inventory glut; revenue −23.2% to $13.27 bn
    2024 : Crolles 300mm JV with GlobalFoundries gets €2.9 bn French state aid approval
    2024 : EU Commission approves €2 bn direct grant for Catania SiC mega-fab (€5 bn project)
    2025 : Revenue troughs at $11.80 bn (−11.1%); $376 m restructuring charge; CEO commits to "high triple-digit million" savings by 2027 exit
    2025 : NXP MEMS-sensor acquisition announced (July 24)
    2025 : Catania granted EU Chips Act "Integrated Production Facility" status (Oct 13)
    2026 : Q1 returns to growth (+23.0% YoY); AWS multi-year multi-$bn AI-datacenter deal (Feb 9)
    2026 : NXP MEMS-sensor acquisition closes (Feb 2)
```
...
```mermaid
graph TD
    ST[STMicroelectronics<br/>FY2025 rev $11.80 bn] --> APMS[APMS Product Group<br/>$6.77 bn / 57%]
    ST --> MDRF[MDRF Product Group<br/>$5.02 bn / 43%]
    APMS --> AMS[AM&S — Analog, MEMS, Sensors<br/>$5.09 bn]
    APMS --> PD[P&D — Power & Discrete<br/>$1.69 bn]
    MDRF --> EMP[EMP — Embedded Processing<br/>$3.58 bn]
    MDRF --> RFOC[RFOC — RF & Optical Comms<br/>$1.44 bn]
    AMS --> AMS1[Smart-power / Analog ICs<br/>BCD, VIPpower]
    AMS --> AMS2[Motion MEMS<br/>accelerometers, gyros]
    AMS --> AMS3[Environmental / bio MEMS]
    AMS --> AMS4[MEMS actuators<br/>print heads, AF, mLED]
    AMS --> AMS5[Optical Sensors<br/>FlightSense ToF, BrightSense, SafeSense]
    AMS --> AMS6[Imaging<br/>iPhone ToF, in-cabin]
    PD --> PD1[Silicon MOSFETs]
    PD --> PD2[SiC MOSFETs / modules]
    PD --> PD3[IGBTs]
    PD --> PD4[Thyristors, rectifiers]
    PD --> PD5[GaN-on-Si power transistors]
    PD --> PD6[ESD / surge protection]
    EMP --> EMP1[STM32 general-purpose MCUs<br/>ARM Cortex-M0 through M85]
    EMP --> EMP2[Stellar automotive MCUs<br/>ASIL-D, zonal, x-in-1]
    EMP --> EMP3[STM32MP microprocessors<br/>Cortex-A35, Linux]
    EMP --> EMP4[Secure MCUs / NFC / eSIM<br/>ST25, STSECURE]
    EMP --> EMP5[Custom ADAS SoCs<br/>Mobileye co-design]
    EMP --> EMP6[EEPROM / non-volatile mem]
    RFOC --> RFOC1[Space-tech ASICs<br/>BiCMOS, FD-SOI for LEO]
    RFOC --> RFOC2[RF / mmWave components<br/>5G beamforming]
    RFOC --> RFOC3[Cloud optical interconnect<br/>SiPho / BiCMOS for AI DC]
    RFOC --> RFOC4[Digital audio amplifiers<br/>car infotainment]
    RFOC --> RFOC5[GNSS positioning ICs<br/>Teseo]
    RFOC --> RFOC6[Wi-Fi / BLE module<br/>ST67W w/ Qualcomm]
```
...
```mermaid
pie title FY2025 Revenue by Top Customers (US$ 11.80 bn)
    "Apple — 17.7%" : 17.7
    "All other (incl. Tesla, Bosch, Denso, Samsung, SpaceX, HP, Continental, Mobileye, Vitesco)" : 82.3
```
...
```mermaid
quadrantChart
    title Analog/MCU/Power semiconductor positioning — Scale vs Margin
    x-axis Lower scale --> Higher scale
    y-axis Lower GM --> Higher GM
    quadrant-1 High scale, high margin
    quadrant-2 Low scale, high margin
    quadrant-3 Low scale, low margin
    quadrant-4 High scale, low margin
    TXN: [0.93, 0.85]
    ADI: [0.75, 0.95]
    NXPI: [0.78, 0.81]
    MCHP: [0.45, 0.85]
