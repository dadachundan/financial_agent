...
![2021–2025 财年收入构成](charts/rmbs_revenue_mix.png)
...
![产品收入趋势 (内存接口芯片)](charts/rmbs_product_revenue.png)
...
![毛利率与营业利润率趋势](charts/rmbs_margin_trend.png)
...
![分地区收入构成](charts/rmbs_geographic_mix.png)
...
![估值同行对比](charts/rmbs_valuation_peers.png)
...
![RMBS 三年股价走势](charts/rmbs_price_3y.png)
...
```mermaid
timeline
    title Rambus 关键里程碑 (1990–2026)
    1990 : Mark Horowitz 与 Mike Farmwald 在加州山景城创立
         : 立项假设——内存带宽将成为 CPU 性能瓶颈
    1997 : 以 12 美元 / 股登陆纳斯达克 (NASDAQ) IPO
    2000 : Intel 采用 RDRAM 配套 Pentium 4 平台
    2003-2010 : 与主要 DRAM 厂商长期专利诉讼周期
    2015 : 从纯授权模式向产品 (芯片) + 安全 IP 战略转型
    2016 : 收购 Smart Card Software (安全)、Bell ID、CryptoResearch Inc.
    2018 : 收购 Hardent (安全 IP)
    2019 : 与 Samsung、SK hynix 长期专利诉讼达成和解
         : 签订多年期授权协议; 现金流趋于稳定
    2020 : 首代 DDR4 内存接口芯片 RCD 放量; 产品收入开始上量
    2021 : Luc Seraphin 出任正式 CEO (自 2017 年起代理)
         : 收购 PLDA Group (PCIe / CXL IP) 与 AnalogX
    2022 : 收购 Hardent——安全 IP
         : DDR5 RCD 套片发布; 首批服务器级 DDR5 型号上市
    2023 : 9 月剥离 SerDes 与 Memory Interface PHY IP 业务
         : 战略聚焦内存接口芯片 + IP + 专利
    2024 : 9 月发布业内首款 HBM4 内存控制器 IP
         : DDR5 RDIMM 8000 (第 5 代 RCD) 与 MRDIMM 12800 发布
    2025 : 产品收入同比 +41% 至 3.48 亿美元
         : HBM4 / HBM4E / PCIe 7.0 / GDDR7 IP 进入交付
    2026 : 4 月任命 Sumeet Gagneja (前 AMD 数据中心 CFO) 为 CFO
         : 3 月发布业内首款 HBM4E 控制器 IP
```
...
```mermaid
graph TD
    A[Rambus Inc.] --> B[内存接口芯片 - 49% 营收]
    A --> C[硅 IP - 11% 营收]
    A --> D[专利授权 - 40% 营收]

    B --> B1[DDR5 RCD - 寄存时钟驱动器]
    B --> B2[MRCD - 复用型寄存时钟驱动器 - MRDIMM]
    B --> B3[MDB - 复用型数据缓冲器]
    B --> B4[CKD - 客户端时钟驱动器, 适配 AI PC / LPDDR5]
    B --> B5[PMIC - 电源管理 IC]
    B --> B6[SPD Hub - 串行存在检测 Hub]
    B --> B7[TS - 温度传感器]
    B --> B8[LPDDR5X SOCAMM2 套片 - 2026 年发布]

    C --> C1[HBM3E / HBM4 / HBM4E 内存控制器 IP]
    C --> C2[GDDR6 / GDDR7 内存控制器 IP]
    C --> C3[PCIe 5.0/6.0/7.0 控制器 / Retimer / 交换芯片 IP]
    C --> C4[CXL 控制器 IP]
    C --> C5[安全 IP - 硬件信任根]
    C --> C6[安全 IP - 高速协议引擎]
    C --> C7[安全 IP - 加密核心]
...
```mermaid
pie title 2025 财年按头部客户收入分布 (百万美元, 估算)
    "Customer A (大概率 Samsung) - 23%" : 163
    "Customer B (大概率 SK hynix) - 18%" : 127
    "Customer C (大概率 Micron) - <10%" : 70
    "其他所有客户" : 348
```
