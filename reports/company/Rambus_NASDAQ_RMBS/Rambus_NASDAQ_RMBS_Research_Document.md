...
![Product revenue trend (Memory Interface Chips)](charts/rmbs_product_revenue.png)
...
![Gross margin and operating margin trend](charts/rmbs_margin_trend.png)
...
![Geographic revenue mix](charts/rmbs_geographic_mix.png)
...
![Valuation peer comparison](charts/rmbs_valuation_peers.png)
...
```mermaid
timeline
    title Rambus key milestones (1990–2026)
    1990 : Founded by Mark Horowitz & Mike Farmwald, Mountain View CA
         : Thesis — memory bandwidth will bottleneck CPU performance
    1997 : NASDAQ IPO at USD 12/share
    2000 : Intel adopts RDRAM for Pentium 4 platforms
    2003-2010 : Long-running patent litigation cycle with major DRAM vendors
    2015 : Strategic pivot to product (chips) & security IP from pure-licensing model
    2016 : Acquires Smart Card Software (security), Bell ID, CryptoResearch Inc.
    2018 : Acquires Hardent (security IP)
    2019 : Settles long-running patent disputes with Samsung & SK hynix
         : Multi-year licensing deals signed; cash flow stabilizes
    2020 : First DDR4 memory interface chip RCDs ramp; product revenue scales
    2021 : Luc Seraphin becomes permanent CEO (acting since 2017)
         : Acquires PLDA Group (PCIe / CXL IP) and AnalogX
    2022 : Acquires Hardent — security IP
         : DDR5 RCD chipset launches; first server DDR5 SKUs
    2023 : Divests SerDes & Memory Interface PHY IP business (Sept)
         : Focus narrows to memory interface chips + IP + patents
    2024 : First HBM4 Memory Controller IP announced (Sept)
         : DDR5 RDIMM 8000 (Gen 5 RCD) & MRDIMM 12800 launched
    2025 : Product revenue +41% to USD 348m
         : HBM4 / HBM4E / PCIe 7.0 / GDDR7 IP shipping
    2026 : Sumeet Gagneja (ex-AMD Data Center CFO) named CFO (April)
         : Industry's first HBM4E Controller IP launched (March)
```
...
```mermaid
graph TD
    A[Rambus Inc.] --> B[Memory Interface Chips - 49% rev]
    A --> C[Silicon IP - 11% rev]
    A --> D[Patent Licensing - 40% rev]

    B --> B1[DDR5 RCD - Registering Clock Driver]
    B --> B2[MRCD - Multiplexed Registering Clock Driver - MRDIMM]
    B --> B3[MDB - Multiplexed Data Buffer]
    B --> B4[CKD - Client Clock Driver for AI PCs / LPDDR5]
    B --> B5[PMIC - Power Management IC]
    B --> B6[SPD Hub - Serial Presence Detect Hub]
    B --> B7[TS - Temperature Sensor]
    B --> B8[LPDDR5X SOCAMM2 chipset - launched 2026]

    C --> C1[HBM3E / HBM4 / HBM4E Memory Controller IP]
    C --> C2[GDDR6 / GDDR7 Memory Controller IP]
    C --> C3[PCIe 5.0/6.0/7.0 Controllers / Retimer / Switch IP]
    C --> C4[CXL Controller IP]
    C --> C5[Security IP - Hardware Roots of Trust]
    C --> C6[Security IP - High-speed Protocol Engines]
    C --> C7[Security IP - Crypto Cores]
    C --> C8[Security IP - Chip Provisioning]

    D --> D1[DDR memory architecture patents]
    D --> D2[High-speed serial link patents]
    D --> D3[Security architecture patents]
```
...
pie title FY2025 revenue by top customers (USD m, est.)
    "Customer A (likely Samsung) - 23%" : 163
    "Customer B (likely SK hynix) - 18%" : 127
    "Customer C (likely Micron) - <10%" : 70
    "All other customers" : 348
```
