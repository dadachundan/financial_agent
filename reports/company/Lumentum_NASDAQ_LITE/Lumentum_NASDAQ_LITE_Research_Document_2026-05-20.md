...
```mermaid
timeline
    title Lumentum — strategic milestones
    2015 : Spin-off from JDSU on Aug 1, 2015
         : Inherits CCOP business — ROADMs, pump lasers, VCSELs
    2017 : VCSEL ramp for Apple iPhone X / Face-ID TrueDepth module
    2018 : Acquires Oclaro for ~$1.8B, adding coherent optics and InP fab
    2019 : Industrial Tech segment expansion via Kaiam asset purchase
    2022 : Acquires NeoPhotonics (Aug 2022, $918M) for high-power tunable & narrow-linewidth lasers
    2023 : Acquires Cloud Light Technology (Hong Kong) for $728.5M
         : Enters AI-datacom transceiver market
    2024 : Reports $546.5M FY24 GAAP loss; restructures coherent DSP / RFIC programs
    2025 : Michael Hurlston named CEO (Feb 7, 2025), succeeding Alan Lowe
         : FY25 revenue +21% to $1.65B; non-GAAP operating margin recovers to 9.7%
    2026 : NVIDIA invests $2.0B (Series A preferred, $695.31/share) + multi-billion supply commitment
         : Q3-FY26 record $808M revenue, 32% non-GAAP OM; OCS backlog $400M+
```
...
```mermaid
graph TD
    A[Lumentum Holdings] --> B[Cloud & Networking]
    A --> C[Industrial Tech]
    B --> B1[Cloud / AI datacom optics]
    B --> B2[Telecom / network optics]
    B --> B3[Component / chip business]
    B1 --> B1a[400G / 800G / 1.6T pluggable transceivers]
    B1 --> B1b[Active optical cables - AOCs]
    B1 --> B1c[Co-packaged optics CPO engines]
    B1 --> B1d[Optical Circuit Switches OCS]
    B2 --> B2a[ROADMs / WSS / line systems]
    B2 --> B2b[Coherent transponders & DWDM modules]
    B2 --> B2c[Pump lasers / EDFA components]
    B2 --> B2d[Submarine / long-haul subsystems]
    B3 --> B3a[EML directly-modulated lasers]
    B3 --> B3b[Tunable / narrow-linewidth lasers]
    B3 --> B3c[InP / Silicon photonic ICs]
    C --> C1[Imaging & sensing - VCSELs]
    C --> C2[Industrial fiber lasers - kW class]
    C --> C3[Ultrafast / picosecond solid-state lasers]
    C --> C4[Diode & gas lasers]
    C1 --> C1a[Apple iPhone Face-ID Tx VCSEL]
    C2 --> C2a[Metal cutting / welding / EV battery]
    C3 --> C3a[Semiconductor microelectronics processing]
```
...
```mermaid
pie title FY2025 net revenue by ship-to country (10-K Item 7)
    "Hong Kong" : 398.6
    "United States" : 312.3
    "Thailand" : 291.8
    "Other Asia-Pacific" : 199.5
    "EMEA" : 163.5
    "Mexico" : 148.5
    "Japan" : 78.3
    "South Korea" : 32.4
