...
```mermaid
xychart-beta
    title "三星 SDI 营收与运营利润 (2021-2025, KRW 万亿 / bn)"
    x-axis ["2021", "2022", "2023", "2024", "2025"]
    y-axis "营收 (KRW 万亿)" 0 --> 25
    bar [13.55, 20.12, 21.44, 16.59, 13.27]
    line [1.07, 1.81, 1.55, 0.36, -1.72]
```
...
```mermaid
timeline
    title 三星 SDI 关键里程碑 1970–2026
    1970 : Samsung-NEC 成立 — 真空管与 CRT
    1984 : 更名 Samsung Electron Devices
    1986 : 进入 LCD 制造
    1998 : 首款圆柱形锂离子电池
    1999 : 更名 Samsung SDI
    2000 : 商用锂电池量产
    2007 : 启动 AMOLED 生产
    2008 : 与 Bosch 合资 SB LiMotive (动力电池, 2012 解散)
    2014 : 合并 Cheil Industries (新材料 / OLED 主体材料)
    2015 : 收购 Magna 电池包业务 (奥地利 Graz)
    2022 : 在 Suwon 启动全固态电池 (ASB) 试产线
    2022 : 与 Stellantis 合资 StarPlus Energy (印第安纳 33 GWh)
    2023 : ASB 首批样品下线,与 BMW 启动验证
    2024 : 与 GM 合资 Synergy Cells (印第安纳 27–36 GWh)
    2024 : Choi Joo-sun (최주선) 出任 CEO,接棒 Park Bum-soo (박범수)
    2025 : Q4 ESS 季度营收创历史新高,BBU 全球份额估算 50%
    2026 : 奔驰长约 USD 6.8bn 签订;Q1 营业亏损收窄至 KRW -156bn
```
...
```mermaid
graph TD
    SDI[三星 SDI] --> BAT[电池板块<br/>FY25 营收 KRW 12.38万亿<br/>占比 93%]
    SDI --> EM[电子材料板块<br/>FY25 营收 KRW 883bn<br/>占比 7%]
    BAT --> EV[EV 电池 / Automotive]
    BAT --> ESS[储能电池 / ESS]
    BAT --> SMALL[小型电池 / Small]
    EV --> EV1["棱柱 P5/P6 NCA 91% Ni"]
    EV --> EV2["46Φ 圆柱 NCA / Mid-Ni"]
    EV --> EV3["ASB 全固态 (2027 量产)"]
    ESS --> ESS1["SBB 1.5 / 1.7 NCA"]
    ESS --> ESS2["SBB 2.0 LFP (2026Q4 美产)"]
    SMALL --> S1["BBU 圆柱 (50% 全球份额)"]
    SMALL --> S2["高功率 Tab-less 圆柱<br/>(电动工具 / HEV)"]
    SMALL --> S3["手机 / IT (Pouch)"]
    EM --> EM1["半导体 — EMC / SOH / SOD / Slurry"]
    EM --> EM2["显示 — OLED PGH / PSF"]
    EM --> EM3["新材 — HBM 封装 / 折叠膜"]
```
...
```mermaid
graph LR
    A[AI 训练负载<br/>GPU Cluster] --> B[BBU 50% 全球份额<br/>圆柱形电芯]
    B --> C[数据中心 BESS<br/>SBB 2.0 LFP]
    C --> D[电网级储能<br/>SBB 1.5 / 1.7 NCA]
    D --> E[EV / HEV<br/>P6 NCA + 46Φ + ASB]
    F[HBM / GPU 封装<br/>EMC / SOH] -.材料.-> A
    G[OLED 显示<br/>PGH / PSF] -.材料.-> A
```
...
```mermaid
pie title FY2025 板块营收构成 (KRW 13.27 万亿)
    "电池 - EV (估算 60%)" : 60
    "电池 - ESS (估算 18%)" : 18
    "电池 - Small / BBU (估算 15%)" : 15
    "电子材料 (实际 7%)" : 7
```
...
```mermaid
quadrantChart
    title 全球电池 / 电力链路竞争象限 2026
    x-axis "EV 单一驱动 (Low)" --> "EV + AI Power 多元化 (High)"
    y-axis "成本领先 (Low)" --> "技术领先 (High)"
    quadrant-1 "技术领先 + 多元化 (SDI 目标位置)"
    quadrant-2 "技术领先 + EV 单一"
    quadrant-3 "成本领先 + EV 单一"
    quadrant-4 "成本领先 + 多元化"
    "三星 SDI": [0.72, 0.78]
    "LG Energy Solution": [0.55, 0.70]
    "CATL": [0.62, 0.65]
    "BYD": [0.50, 0.55]
    "Panasonic": [0.45, 0.68]
    "SK On": [0.30, 0.50]
    "EVE Energy": [0.35, 0.40]
    "Tesla": [0.65, 0.72]
```
