...
```mermaid
timeline
    title TI milestones, 1930–2026
    1930 : Geophysical Service Inc. founded (oilfield seismic)
    1951 : Renamed Texas Instruments Incorporated; pivot to electronics
    ...
    1971 : Single-chip microcomputer (TMS1000) launched
    1996 : Sells defense business to Raytheon for $2.95 bn — refocus on chips
    1997 : Sells DRAM business to Micron — exit commodity memory
    2000 : Acquires Burr-Brown (high-perf analog) for $7.6 bn
    2006 : Sells Sensors & Controls to Bain Capital
    2011 : Acquires National Semiconductor for $6.5 bn (largest deal pre-2026)
    2018 : Rich Templeton hands CEO role to Brian Crutcher (resigns same year); Templeton returns as CEO
    2021 : Breaks ground on Sherman, Texas 300mm fab campus
    2023 : Haviv Ilan becomes CEO; commits $30+ bn six-year capex cycle
    2024 : Receives up to $1.6 bn direct CHIPS Act funding for Sherman & Lehi
    2025 : Sherman SM1 begins production; FY revenue rebounds 13% to $17.68 bn
    2026 : Announces $7.5 bn acquisition of Silicon Labs (Feb); Q1 revenue up 19% YoY
```
...
```mermaid
graph TD
    TXN[Texas Instruments] --> A[Analog 79% of FY2025]
    TXN --> E[Embedded Processing 15%]
    TXN --> O[Other 6%]
    A --> AP[Power]
    A --> AS[Signal Chain]
    AP --> AP1[DC/DC converters & switching regs]
    AP --> AP2[AC/DC & isolated DC/DC]
    AP --> AP3[Battery management]
    AP --> AP4[Linear/LDO regulators]
    AP --> AP5[Multiphase controllers / power stages]
    AP --> AP6[Lighting drivers]
    AS --> AS1[Amplifiers]
    AS --> AS2[Data converters - ADC/DAC]
    AS --> AS3[Interface - USB/Ethernet/CAN]
    AS --> AS4[Motor drivers]
    AS --> AS5[Clocks & timing]
    AS --> AS6[Logic & sensing]
    E --> E1[Microcontrollers MCUs]
    E --> E2[Processors - Sitara/C66x DSP/Jacinto]
    E --> E3[Wireless connectivity - SimpleLink]
    E --> E4[mmWave radar - AWR/IWR]
    O --> O1[DLP imaging]
    O --> O2[Calculators - TI-84 etc.]
    O --> O3[Custom ASICs]
```
...
```mermaid
quadrantChart
    title Analog & Embedded competitive positioning — May 2026
    x-axis Narrow portfolio --> Broad portfolio
    y-axis Lower price/value --> Premium price/performance
    quadrant-1 Premium broad-line
    quadrant-2 Premium specialist
    quadrant-3 Value specialist
    quadrant-4 Value broad-line
    Texas Instruments: [0.9, 0.65]
    Analog Devices: [0.78, 0.85]
    Microchip Technology: [0.7, 0.55]
    STMicroelectronics: [0.85, 0.5]
    NXP Semiconductors: [0.6, 0.7]
    Infineon: [0.7, 0.7]
    Monolithic Power Systems: [0.25, 0.8]
    Silergy: [0.45, 0.35]
```
