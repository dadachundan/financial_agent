...
> **Postscript 6 (added 2026-06-07 late night): v5 adds four more BMC-inspired indicators from free public sources.** After scouting for paywall-free alternatives to the indicators Citi's BMC has but this dashboard doesn't, four became available: **(a) EPS distance from 10y peak** (computed from multpl monthly trailing S&P 500 EPS, 1871-present — today 0%, at the rolling cycle high, amber flag); **(b) FINRA margin debt as ratio to S&P 500 level** (FINRA monthly xlsx, 1997-present — today's ratio sits mid-range, off flag); **(c) US capex YoY growth** (FRED `PNFI` quarterly aggregate private nonresidential fixed investment, 1947-present — today +8.4%, amber flag; Citi's S&P-500-specific capex reads +21% red because it concentrates AI hyperscalers); **(d) CBOE equity put/call ratio** (cdn.cboe.com daily CSV, Nov 2006 – Oct 2019 only — the public CSV is stale, so this is functional for backtest history but reports "n/a current value" for 2026). v5 reads: composite **61.7 / 100, Elevated** and flag count **8.0 / 19** (7 reds + 2 ambers + 10 off). Still below Citi's 10/18 (Global) and 11.5/18 (US) because the remaining missing factors (M&A, IPO, RoE, fund flows, Levkovich) need paid feeds.
